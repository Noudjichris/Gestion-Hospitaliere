    ...
    public partial class DetailsPaiementFrm : Form
    {
                  ...
                  for (var k = 0; k < listeDep.Rows.Count; k++)
                  {
                      ...
                      if (listeMatricule.Count() > 0)
                      {
                          ...
                          foreach (var personnel in listeMatricule)
                          {
                              ...
                              p.SalaireBase = Convert.ToDouble(dtSal.Rows[0].ItemArray[1].ToString());
                              ...
                              var chargeTotal = p.CNPS + p.IRPP + p.ONASA;
                               ...
                               var primes = p.Transport + p.AutresPrimes + p.PrimeGarde + p.PrimeLogement + p.PrimeResponsabilite;
                              ...
                              dataGridView1.Rows.Add(personnel.NumeroMatricule, employe, qualification,
                                  listeEmpl.Rows[0].ItemArray[14].ToString() + "-" + listeEmpl.Rows[0].ItemArray[13].ToString() ,
                                    ...
                                    String.Format(elGR, "{0:0,0}", p.CongeMaternite), 
                              ...
                              totalCoutAbscence += p.CongeMaternite;
                              ...
                              sousTotalCoutAbscence += p.CongeMaternite;
                ...
                if (dataGridView1.Rows.Count > 0)
                {
                    ...
                    if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        {  
                            ...
                            {
                                var div = (dataGridView1.Rows.Count - 20) / 26;

                                for (var i = 0; i <= div; i++)
                                {
                                    if (i * 26 < dataGridView1.Rows.Count)
                                    {
        ...
        private void CmbBank_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (dgRows.Cells[21].Value.ToString().ToUpper() == cmbBank.Text.ToUpper() && !string.IsNullOrEmpty(dgRows.Cells[0].Value.ToString()))
                    {
                        ...
                        personnel.NumeroMatricule = dtPersonnel.Rows[0].ItemArray[0].ToString();
                ...
                var l = from lp in liste
                        orderby lp.Nom
                        select lp;
                var listePersonnel = new List<Personnel>();
                foreach (var p in l)
                {
                ...
                if (listePersonnel.Count > 0)
                {
                    ...
                    if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        var count = listePersonnel.Count;
                        var bitmap = Impression.ImprimerListeDesBancaries(listePersonnel, cmbBank.Text);
                        ...
                        for (var i = 0; i <= div; i++)
                        {
                            if (i * 45 < count-30)
                            {
                                bitmap = Impression.ImprimerListeDesBancaries(listePersonnel, i);
                                inputImage1 = @"cdali" + i;
                                // Create an empty page
        ...
        private void deuxiemeToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }

        void ObtenirIRpp(List<string> listeMois)
        {
            try
            {
                if (dataGridView1.Rows.Count > 0)
        ...
        private void mensuelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.Rows.Count > 0)
        ...
        private void virementBancaireToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                SaveFileDialog sfd = new SaveFileDialog();
                ...
                foreach (DataGridViewRow dgRows in dataGridView1.Rows)
                {
                    if (dgRows.Cells[21].Value.ToString().ToUpper() == cmbBank.Text.ToUpper() && !string.IsNullOrEmpty(dgRows.Cells[0].Value.ToString()))
                    {
                        ...
                        personnel.CodeGuichet = dtPersonnel.Rows[0].ItemArray[24].ToString();
                        personnel.Cle = dtPersonnel.Rows[0].ItemArray[25].ToString();
                        personnel.Telephone1 = Convert.ToDouble(dgRows.Cells[17].Value.ToString()).ToString();
                ...
                dgvView.Columns.Add("no", "N°");
                dgvView.Columns.Add("nom", "Nom & Prenom");
                dgvView.Columns.Add("nomt", "N° Compte");
                dgvView.Columns.Add("nonta", "Code banque");
                dgvView.Columns.Add("nocnp", "Code guichet");
                dgvView.Columns.Add("noas", "Clé");
                dgvView.Columns.Add("noas", "Montant");
                dgvView.Columns.Add("noas", "Date");
                var f=1;
                foreach(var lp in listePersonnel)
                {
                    dgvView.Rows.Add(f, lp.Nom, lp.NumeroCompte, lp.CodeBanque, lp.CodeGuichet, lp.Cle, lp.Telephone1, lp.Telephone2);
                   f++;
                }
                if (sfd.ShowDialog() == DialogResult.OK)
                {
