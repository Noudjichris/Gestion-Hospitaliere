...
namespace SGSP.AppCode
{
    public class ConnectionClass
    {
        ...
        public static bool AjouterUnPersonnel(Personnel personnel, Service service, Salaire salaire)
        {
            ...
            try
            {
                ...
                var requete = "INSERT INTO `pers_tbl` (`num_mat`, `nom`, `prenom`, `date_nai`, `lieu_nai`, `addres`, `tele1`, `tele2`, `email`, `id_dep`, `sexe`, " +
                    "  `photo`,`age`, `no_compte`,code_banc,code_gui,cle) VALUES (@num_mat, @nom, @prenom, @date_nai,@lieu_nai,"+ 
                "@addres, @tele1, @tele2, @email, @id_dep, @sexe, @photo,@age,@no_compte,@code_banc,@code_gui,@cle)";
                ...
                requete = "INSERT INTO `salaire_tbl` (`salaire`, `grille`, `num_mat`, `indice`)" +
                   " VALUES (@salaire, @grille, @num_mat2, @indice)";
        ...
        public static bool ModifierUnPersonnel(Personnel personnel, Service service, Salaire salaire, string ancienMatricule)
        {
            ...
            try
            {
                ...
                var requete = "UPDATE `pers_tbl` SET  `nom` = @nom, `prenom` = @prenom, `date_nai` = @date_nai, `lieu_nai` = @lieu_nai," +
                    ...
                    "`age` = @age, `no_compte` =@no_compte, code_banc=@code_banc,code_gui=@code_gui,cle=@cle , `num_mat`  = @num_mat01  WHERE `num_mat`  = @num_mat";
                ...
                requete =
                    "UPDATE  `salaire_tbl` SET `salaire` =@salaire, `grille` = @grille, `indice` = @indice" +
                    " WHERE num_mat = @num_mat2";
                _command.Parameters.Add(new MySqlParameter("salaire", salaire.SalaireBrut));
                _command.Parameters.Add(new MySqlParameter("grille", salaire.GrilleSalarialle));
                _command.Parameters.Add(new MySqlParameter("num_mat2", personnel.NumeroMatricule));
                _command.Parameters.Add(new MySqlParameter("indice", salaire.Indice));
        ...
        public static bool EtreEnRetraite()
        {
            try
            {
                _connection.Open();
                var requete =
                     "UPDATE `service_tbl` SET  etat=@etat WHERE date_retr<=@date_retr";
                _command.Parameters.Add(new MySqlParameter("etat", true));
                _command.Parameters.Add(new MySqlParameter("date_retr", DateTime.Now.Date));
                _command.CommandText = requete;
                _command.ExecuteNonQuery();
                return true;
            }
    ...
    //supprimer LES PERSONNELS       
     public  static void SupprimerUnPersonnel(string  numeroPersonnel)
        ...
        public static DataTable ListeDesPersonnels()
        {
            ...
            try
            {
                string requete = "SELECT pers_tbl.num_mat, pers_tbl.nom, pers_tbl.prenom, pers_tbl.date_nai, pers_tbl.lieu_nai, " +
                    ...
                    "ON pers_tbl.num_mat = service_tbl.num_mat ORDER BY pers_tbl.nom";
                _command.CommandText = requete;
                _connection.Open();
        ...
        public static DataTable ListeDesPersonnelParNumeroMatricule(string   numMatricule)
        {
            ...
            try
            {
                ...
                "service_tbl.date_fnctr,pers_tbl.code_banc,pers_tbl.code_gui,pers_tbl.cle FROM pers_tbl INNER JOIN dep_tbl ON pers_tbl.id_dep = dep_tbl.id_dep  " +
        ...
        //liste de salaire
        public static DataTable ListeSalaire(string nom ,string prenom)
        {
            DataTable dataTable = new DataTable();
            try
            {
                string requete = "SELECT salaire_tbl.salaire,salaire_tbl.grille,salaire_tbl.indice, "+
                    "pers_tbl.nom, pers_tbl.prenom,salaire_tbl.num_mat FROM pers_tbl INNER JOIN salaire_tbl " +
                   " ON pers_tbl.num_mat = salaire_tbl.num_mat WHERE (pers_tbl.nom = @Param1) AND (pers_tbl.prenom = @Param2)";
                _command.CommandText = requete;
                _connection.Open();
                _command.Parameters.Add(new MySqlParameter("Param1", nom));
                _command.Parameters.Add(new MySqlParameter("Param2", prenom));
                MySqlDataReader reader = _command.ExecuteReader();
                dataTable.Load(reader);
            }
            catch (Exception ex)
        ...
        public static DataTable ListeSalaire(string  matricule)
        {
            var  dataTable = new DataTable();
            try
            {
                var  requete =
                  " SELECT salaire_tbl.salaire,salaire_tbl.grille,salaire_tbl.indice, pers_tbl.nom, pers_tbl.prenom FROM pers_tbl INNER JOIN salaire_tbl " +
                   " ON pers_tbl.num_mat = salaire_tbl.num_mat WHERE salaire_tbl.num_mat =@num_mat ";
                _command.CommandText = requete;
        ...
        public static bool EnregistrerUnConge(Conge conge, string etat)
        {
            ...
            try
            {
                ...
                if (etat  == "1")
                {
                    requete = "INSERT INTO conge_tbl(`num_mat` ,`date_dep` ,`date_rep` , `nature_con`,`date_dem`) " +
                    "VALUES(@num_mat ,@date_dep ,@date_rep ,@nature_con, @date_dem)";
                }
                else
                {
                    requete = "UPDATE conge_tbl SET  `date_dep` = @date_dep ,`date_rep` = " +
                        "@date_rep ,`nature_con` = @nature_con WHERE `id_conge` = " + conge.IDConge;
                }
                ...
                _command.Parameters.Add(new MySqlParameter("date_dem", DateTime.Now));
                _command.CommandText = requete;
                _connection.Open();
                _command.ExecuteNonQuery();
        ...
        public static List<Conge> ListeConge()
        {
            ...
            try
            {
                string requete = "SELECT conge_tbl.*,pers_tbl.nom, pers_tbl.prenom" +
                    " FROM conge_tbl JOIN pers_tbl ON conge_tbl.num_mat=pers_tbl.num_mat";
                _command.CommandText = requete;
                _connection.Open();
                MySqlDataReader reader = _command.ExecuteReader();
                while (reader.Read())
                {
                    ...
                    conge.DateDemande = reader.GetDateTime(4);
                    conge.NatureConge = !reader.IsDBNull(5) ? reader.GetString(5) : "";
                    conge.NomPersonnel = reader.GetString(6) + " " + reader.GetString(7);
                    liste.Add(conge);
                }
            }
            finally
        ...
        //ajouter une absence
        public static bool EnregistrerUneAbsence(Absence abscence, string etat)
        {
            var flag = false;
            try
            {
                ...
                if (etat == "1")
                {
                     requete = "INSERT INTO `absence_tbl`(`num_mat`, `date_abs`,  `date_rep`, `motif`,`date_deman`)" +
                        "VALUES (@num_mat,@date_abs,@date_rep, @motif,@date_deman)";
                }
                else
                {
                     requete = "UPDATE absence_tbl SET  `date_abs` = @date_abs ,`date_rep` =@date_rep " +
                   " ,`motif` = @motif, num_mat=@num_mat WHERE `id_absc` = "+ abscence.IDAbsence;
                }
                _command.Parameters.Add(new MySqlParameter("num_mat", abscence.NumeroMatricule));
                _command.Parameters.Add(new MySqlParameter("date_abs", abscence.DateDebutAbscense));
                _command.Parameters.Add(new MySqlParameter("date_rep", abscence.DateRetour));
                _command.Parameters.Add(new MySqlParameter("motif", abscence.Motif));
                _command.Parameters.Add(new MySqlParameter("date_deman", DateTime.Now));
                _command.CommandText = requete;
                _connection.Open();
                _command.ExecuteNonQuery();
                MonMessageBox.ShowBox("L'enregistrement est fait avec succés", "Information");
      ...
      //supprimer une demande de absences
        ...
        public static List<Absence> ListeUneAbsence()
        {
            ...
            try
            {
                string requete = "SELECT absence_tbl.*,pers_tbl.nom, pers_tbl.prenom"+
                    " FROM absence_tbl JOIN pers_tbl ON absence_tbl.num_mat=pers_tbl.num_mat";
                _command.CommandText = requete;
                _connection.Open();
                MySqlDataReader reader = _command.ExecuteReader();
                while (reader.Read())
                {
                    ...
                    absence.NumeroMatricule = reader.GetString(1);
                    absence.DateDebutAbscense = reader.GetDateTime(2);
                    absence.DateRetour = reader.GetDateTime(3);
                    absence.DateDemande = reader.GetDateTime(4);
                    absence.Motif = !reader.IsDBNull(5) ? reader.GetString(5) : "";
                    absence.NomPersonnel = reader.GetString(6) +" " + reader.GetString(7) ;
        ...
        //liste des absences trie par numatricule
        public static DataTable ListeUneAbsenceParMatricule(int numerMatricule)
        {
            DataTable dataTable = new DataTable();
            try
            {
                string requete = "SELECT * FROM absence_tbl WHERE num_mat = " + numerMatricule + " ORDER BY id_absc DESC";
                _command.CommandText = requete;
                _connection.Open();
                MySqlDataReader reader = _command.ExecuteReader();
                dataTable.Load(reader);
        ...
        //liste des absences trie par numatricule
        public static DataTable ListeUneAbsence(int numeroAbsence)
        {
            DataTable dataTable = new DataTable();
            try
            {
                string requete = "SELECT * FROM absence WHERE id_absc = " + numeroAbsence + " ORDER BY id_absc DESC";
                _command.CommandText = requete;
                _connection.Open();
                MySqlDataReader reader = _command.ExecuteReader();
                dataTable.Load(reader);
        ...
        public static int  CreerUnOrdreDePaiement(Paiement paiement)
        {
            try
            {
                _connection.Open();
        ...
        public static bool ModifierOrdreDePaiement(Paiement paiement,  DataGridView dgvPaiement)
        {
            bool flag = false;
            try
            {
                _connection.Open();
                _transaction = _connection.BeginTransaction();
                string requete = "UPDATE ordre_pai SET `montant_total` = @montant_total,`date_ord`= @date_ord,"+
                    "`exercice`="+paiement.Exercice  +" WHERE `id_paie` = " + paiement.IDPaie ;
                _command.Parameters.Add(new MySqlParameter("montant_total", paiement.MontantTotal));
                _command.Parameters.Add(new MySqlParameter("date_ord", paiement.DatePaiement));
                //_command.Parameters.Add(new MySqlParameter("mois", paiement.MoisPaiement));
                _command.CommandText = requete;
                _command.Transaction = _transaction;
                _command.ExecuteNonQuery();

                foreach (DataGridViewRow dgvRow in dgvPaiement.Rows)
                {
                                 requete = "UPDATE paimn_tbl SET  num_mat,prime_perf,prime_rend,heure_supp,prime_grd,salaire_brut," +
                            " salaire_net,acompte_paye,siPaye,modePaiement,date_pai,taux_cnps,idAcpt) VALUES( @id_pai, @num_mat,@prime_perf,@prime_rend,@heure_supp," +
                        " @prime_grd,@salaire_brut,@salaire_net,@acompte_paye,@siPaye,@modePaiement,@date_pai,@cnps,@idAcpt)";
                        ...
                        _command.Parameters.Add(new MySqlParameter("prime_perf", paiement.PrimeResponsabilite));
                        _command.Parameters.Add(new MySqlParameter("prime_rend", paiement.PrimeLogement));
                        _command.Parameters.Add(new MySqlParameter("heure_supp", paiement.HeureSupplementaire));
                        _command.Parameters.Add(new MySqlParameter("prime_grd", paiement.PrimeGarde));
                        _command.Parameters.Add(new MySqlParameter("date_pai", paiement.DatePaiement));
                        ...
                        //_command.Parameters.Add(new MySqlParameter("siPaye", paiement.Etat));
                        _command.Parameters.Add(new MySqlParameter("modePaiement", paiement.ModePaiement));
                        _command.Parameters.Add(new MySqlParameter("cnps", paiement.CNPS));
                        ...
                        _command.Parameters.Add(new MySqlParameter("id_transp", paiement.Transport));
        ...
        public static bool ModifierOrdreDePaiement(DataGridView dgvPaiement)
        {
            bool flag = false;
            try
            {
                ...
                foreach (DataGridViewRow dgvRow in dgvPaiement.Rows)
                {
                   var  requete = "UPDATE paimn_tbl SET  salaireBase=@salaireBase WHERE  num_mat=@num_mat";
                    _command.Parameters.Add(new MySqlParameter("salaireBase",double.Parse( dgvRow.Cells[2].Value.ToString())));
                    _command.Parameters.Add(new MySqlParameter("num_mat", dgvRow.Cells[0].Value.ToString()));
                    _command.Transaction = _transaction;
                    _command.CommandText = requete;
                    _command.ExecuteNonQuery();
                    _command.Parameters.Clear();
        ...
        public static void SupprimerOrdreDePaiement(int numPaiement)
        {
            ...
            {
                _connection.Open();
                 var requete = "DELETE FROM ordre_pai  WHERE (id_paie = " + numPaiement + ")";
                    _command.CommandText = requete;
                    _command.ExecuteNonQuery();
                    MonMessageBox.ShowBox("La suppression des données est faite avec succés", "Suppression paiement");
            ...
            try
            {
                ...
                if (!reader.HasRows)
                {
                    ...
                    requete = "INSERT INTO paimn_tbl( id_pai, num_mat,conge, cout_absc, salaire_brut,taux_cnps,irpp,onasa,charge_pat,prime_loge"+
                                        ",prime_grd,prime_respo,heure_supp,ind_transp,acompte_paye,avanceSa,soinFami,salaire_net,modePaiement,idAcpt,cout_salariale,salaireBase,jour_pres)" +
                                       " VALUES(  @id_pai, @num_mat,@conge, @cout_absc, @salaire_brut,@taux_cnps,@irpp,@onasa,@charge_pat,@prime_loge"+
                                        ",@prime_grd,@prime_respo,@heure_supp,@ind_transp,@acompte_paye,@avanceSa,@soinFami,@salaire_net,@mode_paiement,@idAcpt,@cout_salariale,@salaireBase,@jour_pres)";
                    _command.Parameters.Add(new MySqlParameter("id_pai", paiement.IDPaie));
                    _command.Parameters.Add(new MySqlParameter("num_mat", paiement.NumeroMatricule));
                    _command.Parameters.Add(new MySqlParameter("conge", paiement.CongeAnnuel));
        ...
        //supprimer un ordre de paiement       
        public static bool ProcederAuPaiement(string  numeroMatricule, int numPaiement,bool etat)
        {
            try
            {
                _connection.Open();
                 var  requete = "UPDATE paimn_tbl SET `siPaye` = @etat , date_pai = @date_pai WHERE `id_pai` = " 
                       + numPaiement+" AND num_mat = @num_mat" ;
                    _command.Parameters.Add(new MySqlParameter("etat", etat));
                    _command.Parameters.Add(new MySqlParameter("num_mat", numeroMatricule));
                    _command.Parameters.Add(new MySqlParameter("date_pai", DateTime.Now));
                    _command.CommandText = requete;
                    _command.ExecuteNonQuery();
                return true;
            }
            ...
            {
                MonMessageBox.ShowBox("La modification des données du personnel a échoué", "Suppression paiement", exception);
                return false;
            }
            finally
            {
        ...
        public static List<Paiement> ListeDetailsPaiement(int numeroPaiement)
        {
            ...
            {   var liste = new List<Paiement>();
                ...
                while (reader.Read())
                {
                    ...
                    p.HeureSupplementaire=reader.GetDouble(13);
        ...
        public static bool EnregistrerRecapitulatif(Paiement p, string state)
        {
            try
            {
                ...
                if (state == "1")
                {
                    ...
                    _command.Parameters.Add(new MySqlParameter("libelle",p.LibeelleRecap));
        ...
        public static List<Paiement> ListeRecapitulatifs(int numeroPaiement)
        {
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    p.LibeelleRecap = reader.GetString(2);
                    p.IDPaie = reader.GetInt32(1);
