    ...
    public partial class JournalDesRecettesFrm : Form
    {
        ...
        private void JournalDesRecettesFrm_Load(object sender, EventArgs e)
        {
            ...
            if(state=="2")
            {
                ...
                dtp1.Value = dateDebut;
                dtp2.Value = dateFin;
                dtp1.Enabled = false;
                dtp2.Enabled = false;
                Text = "Journal des recettes de " + entreprise;
            }
        }
        ...
        public string state,entreprise; public double montantPharmacie;
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (checkBox1.Checked)
                {
                    RecettesJournalieresDeLaCaisseEtPharmacieConventionnes();
                }
                else if (checkBox2.Checked)
                {
                    RecettesMensuellesDeLaCaisseEtPharmacieConventionnes();
                }
            }
        }
                ...
                foreach (var jour in listeDesJour)
                {
                  var  totalRecettesCaisseduJour = .0;
                    ...
                    foreach (var libelle in listeLibelle)
                    {
                        ...
                        var requete1 = "SELECT  COUNT(det_fact.design)  FROM det_fact INNER JOIN" +
                                                " facture_tbl ON det_fact.id_fact = facture_tbl.id_fact WHERE det_fact.groupage LIKE '" + libelle.Designation + "%' " +
                                           "AND facture_tbl.date_fact >= @date1 AND facture_tbl.date_fact < @date2 GROUP BY   det_fact.design";
                        var dtParLib = AppCode.ConnectionClassClinique.TableFacture(requete1, jour, jour.AddHours(24));

                       //if( dtParLib.Rows.Count>0)
                        {
                        ...
                        {
                            var montantPharmacie = AppCode.ConnectionClassPharmacie.ListeDesVentes(jour, jour);
                                var solde = .0;
                               
                                 var dtFact = AppCode.ConnectionClassPharmacie.ListeDesFactures("",jour, jour.AddHours(24));
                                for (var l = 0; l < dtFact.Rows.Count; l++)
                                {
                                    solde += double.Parse(dtFact.Rows[l].ItemArray[3].ToString());
                                }
                                montantPharmacie -= solde;
                               dgvRecettes.Rows.Add(
                        ...
                        else if (libelle.Designation.ToUpper() == "HOSPITALISATION")
                        {
                            var sousTotalRecettesCaisse = .0;
                            var dtHos = AppCode.ConnectionClassPharmacie.ListeDesVentesDataTable(jour, jour);
                    ...
                    foreach (var libelle in listeLibelle)
                    {
                        var montantRecettesCaisse = 0.0;
                         var requete1 = "SELECT  COUNT(det_fact.design)  FROM det_fact INNER JOIN" +
                                                " facture_tbl ON det_fact.id_fact = facture_tbl.id_fact WHERE det_fact.groupage LIKE '" + libelle.Designation + "%' " +
                                           "AND facture_tbl.date_fact >= @date1 AND facture_tbl.date_fact < @date2 GROUP BY   det_fact.design";
                         var dtParLib = AppCode.ConnectionClassClinique.TableFacture(requete1, dateDebut, dateFin.AddHours(24));
                       ...
                       //if( dtParLib.Rows.Count>0)
                       {
                        ...
                        if (libelle.Designation == "Vente médicaments")
                        {
                            var montantPharmacie = AppCode.ConnectionClassPharmacie.ListeDesVentes(dateDebut, dateFin);
                            var solde = .0;

                            var dtFact = AppCode.ConnectionClassPharmacie.ListeDesFactures("", dtp1.Value.Date, dtp2.Value.Date.AddHours(24));
                            for (var l = 0; l < dtFact.Rows.Count; l++)
                            {
                                solde += double.Parse(dtFact.Rows[l].ItemArray[3].ToString());
                            }
                        ...
                        {
                            var sousTotalRecettesCaisse = .0;
                            var dtHos = AppCode.ConnectionClassPharmacie.ListeDesVentesDataTable(dateDebut, dateFin);
        ...
        void RecettesJournalieresDeLaCaisseEtPharmacieConventionnes()
        {
            try
            {
                var listeDesJour = new List<DateTime>();
                ...
                foreach (var jour in listeDesJour)
                {
                    ...
                    foreach (var libelle in listeLibelle)
                    {
                        ...
                        if (libelle.Designation == "Vente médicaments")
                        {
                            ...
                            var requete = "SELECT  pro_det_fact.design, COUNT(pro_det_fact.design) AS Expr2, SUM(pro_det_fact.prix_total ) FROM pro_det_fact INNER JOIN" +
                                                " pro_facture_tbl ON pro_det_fact.id_fact = pro_facture_tbl.id_fact INNER JOIN patient_tbl ON pro_facture_tbl.num_patient="+
                                                "patient_tbl.id WHERE(pro_det_fact.groupage = 'Vente médicaments') AND pro_facture_tbl.date_fact >= @date1 AND " +
                                                " pro_facture_tbl.date_fact < @date2 AND patient_tbl.entrep='"+entreprise+"' GROUP BY   pro_det_fact.design";
                            var dtCaisse = AppCode.ConnectionClassClinique.TableFacture(requete, jour, jour.AddHours(24));
                        ...
                        else if (libelle.Designation == "Consultation")
                        {
                            ...
                            var requete = "SELECT  pro_det_fact.design, COUNT(pro_det_fact.design) AS Expr2, SUM(pro_det_fact.prix_total ) FROM pro_det_fact INNER JOIN" +
                                        " pro_facture_tbl ON pro_det_fact.id_fact = pro_facture_tbl.id_fact INNER JOIN patient_tbl ON pro_facture_tbl.num_patient=" +
                                        "patient_tbl.id WHERE(pro_det_fact.groupage = 'Consultation') AND pro_facture_tbl.date_fact >= @date1 AND " +
                                        " pro_facture_tbl.date_fact < @date2 AND patient_tbl.entrep='" + entreprise + "' GROUP BY   pro_det_fact.design";
                        ...
                        else if (libelle.Designation.Contains("Laboratoire"))
                        {
                            ...
                            {
                                montantRecettesCaisse = 0;
                                var liste = from re in AppCode.ConnectionClassClinique.RecettesLaboratoireConventionne(jour, jour,entreprise)
                                            where re.Sub.ToLower().Contains(li.ToLower())
                                            select re;
                                foreach (var re in liste)
                                {
                        ...
                        else
                        {
                            ...
                            var requete = "SELECT  pro_det_fact.design, COUNT(pro_det_fact.design) AS Expr2, SUM(pro_det_fact.prix_total ) FROM pro_det_fact INNER JOIN" +
                                       " pro_facture_tbl ON pro_det_fact.id_fact = pro_facture_tbl.id_fact INNER JOIN patient_tbl ON pro_facture_tbl.num_patient=" +
                                       "patient_tbl.id WHERE(pro_det_fact.groupage = '" + libelle.Designation + "') AND pro_facture_tbl.date_fact >= @date1 AND " +
                                       " pro_facture_tbl.date_fact < @date2 AND patient_tbl.entrep='" + entreprise + "' GROUP BY   pro_det_fact.design";
        ...
        void RecettesMensuellesDeLaCaisseEtPharmacieConventionnes()
        {
            try
            {
                ...
                foreach (var libelle in listeLibelle)
                {
                    ...
                    else if (libelle.Designation.Contains("Laboratoire"))
                    {
                        ...
                        {
                            montantRecettesCaisse = 0;
                            var liste = from re in AppCode.ConnectionClassClinique.RecettesLaboratoireConventionne(dateDebut, dateFin,entreprise)
                                        where re.Sub.ToLower().Contains(li.ToLower())
                                        select re;
                            foreach (var re in liste)
                            {
