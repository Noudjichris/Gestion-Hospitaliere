...
namespace GestionPharmacetique.AppCode
{
    public class ConnectionClass
    {
        ...
        public static List<Medicament> ListeDesLotsProduitsParCode(string Code)
        {
            ...
            try
            {
                string requete = "SELECT * FROM  lot_tbl WHERE num_prod ='" + Code + "' WHERE qte>0  ORDER BY `date`";
                command.CommandText = requete;
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
    ...
    "Information modification", "affirmation.png");
      ...
      " WHERE detail_vente.date_vente>=@date_vente1 AND detail_vente.date_vente <@date_vente2 ORDER BY medicament.nom_medi, detail_vente.date_vente DESC, detail_vente.heure DESC";
        ...
        //Rechercher vente par date
        public static DataTable ListeDesVentes(bool  state ,string nomEmploye, DateTime date1, DateTime date2)
        {
            DataTable dtVente = new DataTable();
            try
            {
                string requete = "SELECT medicament.nom_medi, vente.prix_vente, SUM(vente.quantite) , SUM(vente.quantite * vente.prixTotal )," +
                    ...
                    "%' AND (detail_vente.date_vente >= @date_vente1 AND detail_vente.date_vente < @date_vente2) AND detail_vente.etat=@state GROUP BY medicament.nom_medi, vente.prix_vente ";
                connection.Open();
                command.Parameters.Add(new MySqlParameter("date_vente1", date1));
        ...
        public static bool EnregistrerFacture(double montant, string numEmploye, DateTime date)
        {
            bool flag = false;
            try
            {
                connection.Open();
                _transaction = connection.BeginTransaction();
                string requete = string.Format("SELECT * FROM `facture_tbl` WHERE num_empl=@numEmpl AND date_fact=@date_fac");
                command.Parameters.Add(new MySqlParameter("numEmpl", numEmploye));
                command.Parameters.Add(new MySqlParameter("date_fac", date));
                command.CommandText = requete;
                ...
                if (dt.Rows.Count <= 0)
                {
                    requete = string.Format("INSERT INTO `facture_tbl` (  `date_fact`, `montant`,  `num_empl`)   VALUES (@date_fact, @montant, @num_empl)");
                    command.Parameters.Add(new MySqlParameter("date_fact", date));
                    command.Parameters.Add(new MySqlParameter("montant", montant));
                    command.Parameters.Add(new MySqlParameter("num_empl", numEmploye));
                    command.CommandText = requete;
                    command.Transaction = _transaction;
                    command.ExecuteNonQuery();
                }
                ...
                {
                    requete = string.Format("UPDATE  `facture_tbl` SET  `montant`=@montant WHERE `num_empl`=@num_empl AND `date_fact`=@date_fact");
                    command.Parameters.Add(new MySqlParameter("date_fact", date));
                    command.Parameters.Add(new MySqlParameter("montant", montant));
                    command.Parameters.Add(new MySqlParameter("num_empl", numEmploye));
                    command.CommandText = requete;
                ...
                _transaction.Commit();
                flag = true;
                MonMessageBox.ShowBox(" données Mises à jour avec succés ", "Information Insertion", "affirmation.png");
                //lstVente.Items.Clear();
       ...
       #region Encaissement
        ...
        public static DateTime ObtenirDebutJour(int mois)
        {
            return DateTime.Parse("01/" + mois + "/" + DateTime.Now.Year);
        }

        public static DateTime ObtenirFinJour(int mois)
        {
            if (mois == 1)
            {
