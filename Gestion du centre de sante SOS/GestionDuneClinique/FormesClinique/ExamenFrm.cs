            ...
            try
            {
                ...
                if (dgvAnal.Rows.Count > 0)
                {
                    ...
                    foreach (var a in listeFacture)
                        numeroFacture = a;
                    analyseBitmap = Impression.ImprimerAnalyse(numeroFacture, patient, dateTimePicker1.Value, GestionAcademique.LoginFrm.nom,1,"Examen");
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                if (ListeExamFrm.ShowBox() == "1")
                {
                    ...
                    //           t.NumeroActe == idAnalyse).Where(p => p.Sub == "EXAMEN");
                    ////var netPayer = .0;
