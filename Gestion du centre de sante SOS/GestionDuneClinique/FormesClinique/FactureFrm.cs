               ...
               try
            {
                if (txtPatient.Text !="")
                {
                        if (Double.TryParse(txtPaye.Text, out montantPaye))
                        {
                            ...
                            {
                                if(!ConnectionClassClinique.ListeDesPatients(idPatient)[0].Couvert)
                                {
                                    MonMessageBox.ShowBox("Ce patient n'est pas couvert veuillez decocher la case conventionné avant de proceder à l'enregistrement", "Erreur", "erreur.png");
                                    return ;
                                }
                            }
                        ...
                        facture.NumeroFacture = numFacture;
                            ...
                            facture.Reste = reste;
                            facture.NumeroActe = numeroActe;
                            facture.Acte = typeActe;
                            if (etat == "1")
                            {
                                if (ConnectionClassClinique.EnregistrerUneFacture(facture, dataGridView1,numeroActe,typeActe, montantPaye, checkBox2.Checked))
                                {
