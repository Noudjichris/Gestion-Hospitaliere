    ...
    public partial class ExamenFrm : Form
    {
        ...
        public static string ShowBox()
        {
            try
            {
                ...
                //frmList.Size = new Size(GestionAcademique.Form1.width, GestionAcademique.Form1.height);
                frmList.txtRechercheExamen.Focus();
                frmList.ShowDialog();
            }
        ...
        private void ExamenFrm_Load(object sender, EventArgs e)
        {
            ...
            cmbMedecin.Items.Remove("PARTENAIRE EXTERNE");
            cmbMedecin.Items.Add("PARTENAIRE EXTERNE");
            button8.Focus();
            Location = new Point(GestionAcademique.Form1.xLocation, GestionAcademique.Form1.yLocation);
        }
        ...
        private void ListesDesExamens()
        {
            try
            {
                ...
                var listeAnalyse = from a in ConnectionClassClinique.ListePrixDesAnalyses()
                                   where a.TypeAnalyse.ToUpper().Contains(txtRechercheExamen.Text.ToUpper())
                                   where a.IndexPrix == indexPrix
                                   orderby a.TypeAnalyse
                                   select a;
                var lll = listeAnalyse.Count();
                foreach (var analyse in listeAnalyse)
                {
                    dataGridView1.Rows.Add(analyse.NumeroListeAnalyse, analyse.TypeAnalyse, analyse.Frais);
                }
            }
            catch {  }
        }
            ...
            try
            {
                ...
                if (dgvAnal.Rows.Count > 0)
                {
                    ...
                    foreach (var p in listePatient)
                        patient = p;
                    analyseBitmap = Impression.ImprimerAnalyse(idAnalyse, dgvAnal, patient, dateTimePicker1.Value, GestionAcademique.LoginFrm.nom);
                    ...
                    return;
                        ...
                        montantTotal = Double.Parse(txtmontanttotal.Text);
                             ...
                             if (etat == "1")
                            {
                                ...
                                analyse.Partenaire = partenaire;
                                if (ConnectionClassClinique.AjouterLesAnalysesEffectues(analyse, dgvAnal, nomPatient))
                                {
                                    idExamen = ConnectionClassClinique.ObtenirDernierNumeroAnalyse(); 
        ...
        {
            e.Graphics.DrawImage(analyseBitmap, 5, 10, analyseBitmap.Width, analyseBitmap.Height);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                if (ListeExamFrm.ShowBox() == "1")
                {
                    ...
                    //if (dtAnalyse.Rows[0].ItemArray[4].ToString().ToUpper() == "PARTENAIRE EXTERNE")
                    //{
                    //    txtPartenaire.Text = dtAnalyse.Rows[0].ItemArray[6].ToString().ToUpper();
                   ...
                   dataGridView2.SelectedRows[0].Cells[2].Value.ToString();
                                   ...
                                   bool.Parse(dataGridView2.SelectedRows[0].Cells[5].Value.ToString()))
                    {
                        ...
                        if (ConnectionClassClinique.BondesExamens(analyse, dgvAnal))
                        {
                            ...
                            foreach (var p in listePatient)
                                patient = p;
                            analyseBitmap = Impression.ImprimerAnalyse(idExamen, dgvAnal, patient, dateTimePicker1.Value, GestionAcademique.LoginFrm.nom);
                            if (printDialog1.ShowDialog() == DialogResult.OK)
                            {
                                printDocument1.PrinterSettings = printDialog1.PrinterSettings;
                                printPreviewDialog1.ShowDialog();
