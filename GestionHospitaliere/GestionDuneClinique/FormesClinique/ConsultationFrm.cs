            ...
            try
            {
                if (dgvConsult.SelectedRows.Count > 0)
                {
                    ...
                    foreach (var a in listeFacture)
                        numeroFacture = a;
                    bitmap = Impression.ImprimerAnalyse(numeroFacture, patient, dateTimePicker1.Value, GestionAcademique.LoginFrm.nom,1);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(cmbType.Text))
                {
                    ...
                    //if ((montantMensuelParPatient + frais >= montantLimite) && siLimit)
                    //{
