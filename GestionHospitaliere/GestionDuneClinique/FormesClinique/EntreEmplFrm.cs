...
namespace GestionDuneClinique.FormesClinique
{
    public partial class EntreEmplFrm : Form
    {
        ...
        {
            // TODO: This line of code loads data into the 'clinique_dbDataSet.det_anal' table. You can move, or remove it, as needed.
            //this.det_analTableAdapter.Fill(this.clinique_dbDataSet.det_anal);
            txtAge.Text = "adulte";
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            if (txtNom.Text != "" && txtPrenom.Text != "")
            {
                ...
                if (cmbEntrep.Text != "")
                {
                    ...
                    if (cmbEntrep.Text == "HNDA")
                    {
                        var dt = AppCode.ConnectionClassSGSP.ListeDesPersonnelParNomPersonnel("");
                        var count = 0;
                        foreach (DataRow dtRow in dt.Rows)
                        {
                            if (dtRow.ItemArray[0].ToString() == txtMatricule.Text)
                            {
                                count++;
                            }
                        }
                        if (count == 0)
                        {
                            MonMessageBox.ShowBox("Le matricule n'est pas correct. Veuillez selectionnersur la liste des employés", "Erreur","erreur.png");
                            return;
                        }

                        var listeEmpl = from l in AppCode.ConnectionClassClinique.ListeDesEmployeesEntreprise()
                                        join en in AppCode.ConnectionClassClinique.ListeDesEntreprises()
                                        on l.IdEntreprise equals en.NumeroEntreprise
                                        where en.NomEntreprise ==cmbEntrep.Text
                                        select new { l.Matricule };
                        var countMatricle = 0;
                        foreach (var l in listeEmpl)
                        {
                            if (l.Matricule== txtMatricule.Text)
                            {
                                countMatricle++;
                            }
                        }
                    ...
                    patient.Matricule = txtMatricule.Text;
                    var listeempl = ConnectionClassClinique.ListeDesEmployeesEntreprise(idEntrep, textBox1.Text);
                    ListeEmploye(listeempl);
