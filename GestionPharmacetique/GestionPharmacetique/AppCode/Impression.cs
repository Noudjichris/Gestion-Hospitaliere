...
namespace GestionPharmacetique.AppCode
{
    ...
    string nomFournisseur, string dateLivraison, int start)
        ...
        //    int detail_hauteur_facture = 22 * unite_hauteur;
        ...
        //    Image logo = global::GestionPharmacetique.Properties.Resources.logo;
        //    graphic.DrawImage(logo, 23, 18, 17 * unite_largeur + 0, 5 * unite_hauteur);
        ...
        //}
        //fonction pour dessiner le stock inventaire
        public static Bitmap ImprimerFacture(int numVente, DataGridView dgvVente, Client client, DateTime dateVente, string titre, string typeVente)
        {
            //les dimension de la facture
            #region
            ...
            int detail_hauteur_facture = 19 * unite_hauteur;
            int hauteur_facture = 34 * unite_hauteur;
            ...
            Font fnt2 = new Font("Arial Narrow", 15, FontStyle.Bold | FontStyle.Underline);
            Font fnt33 = new Font("Arial Narrow", 11, FontStyle.Bold);
            try
            {
                //Image logo = global::GestionPharmacetique.Properties.Resources.imageFond;
                //graphic.DrawImage(logo, 1 * unite_largeur + 20, 2 * unite_hauteur, 14 * unite_largeur, 35 * unite_hauteur);

              var  logo = global::GestionPharmacetique.Properties.Resources.logo;
                graphic.DrawImage(logo, 20, unite_hauteur, 16 * unite_largeur, 4 * unite_hauteur);
                graphic.DrawString("CAISSIER(E) " + GestionPharmacetique.Form1.nomEmploye, fnt1, Brushes.Black, 7 * unite_largeur, 32 * unite_hauteur + 10);

            }
            catch (Exception)
            { }
            //graphic.DrawString("Date : " + dateVente.ToShortDateString(), fnt3, Brushes.DeepSkyBlue, 10, 13 * unite_hauteur);
            graphic.DrawString(titre.ToUpper() + numVente, fnt2, Brushes.Black, unite_largeur, 7 * unite_hauteur + 15);
            graphic.DrawString("Date : " + dateVente, fnt1, Brushes.Black, 11 * unite_largeur, 7 * unite_hauteur + 15);

            graphic.DrawString("IDENTIFIANT : " + client.Matricule, fnt33, Brushes.Black, unite_largeur, 5 * unite_hauteur + 10);

            if (!string.IsNullOrEmpty(client.Entreprise))
            {
                graphic.DrawString("INSTITUTION : " +client.Entreprise, fnt1, Brushes.Black, 8 * unite_largeur, 5 * unite_hauteur + 10);

            }
            if (client.NomClient == "CLIENT COMPTANT")
            {
                graphic.DrawString("Nom client : - ".ToUpper(), fnt1, Brushes.Black, unite_largeur, 6 * unite_hauteur + 10);
            }
            else
            {
                graphic.DrawString("Nom client : ".ToUpper() + client.NomClient.ToUpper(System.Globalization.CultureInfo.CurrentCulture), fnt1, Brushes.Black, unite_largeur, 6 * unite_hauteur + 10);
            }


            graphic.DrawRectangle(Pens.SteelBlue, 25, 8 * unite_hauteur + 20, 16 * unite_largeur, 25);
            graphic.FillRectangle(Brushes.SteelBlue, 25, 8 * unite_hauteur + 20, 16 * unite_largeur, 25);
            graphic.DrawRectangle(Pens.SteelBlue, 25, 9 * unite_hauteur + 20, unite_largeur, detail_hauteur_facture);
            graphic.DrawRectangle(Pens.SteelBlue, 25, 9 * unite_hauteur + 20, 10 * unite_largeur - 10, detail_hauteur_facture);
            graphic.DrawRectangle(Pens.SteelBlue, 25, 9 * unite_hauteur + 20, 12 * unite_largeur - 3, detail_hauteur_facture);
            graphic.DrawRectangle(Pens.SteelBlue, 25, 9 * unite_hauteur + 20, 13 * unite_largeur + 10, detail_hauteur_facture);
            graphic.DrawRectangle(Pens.SteelBlue, 25, 9 * unite_hauteur + 20, 16 * unite_largeur, detail_hauteur_facture);

            graphic.DrawString("No", fnt3, Brushes.White, unite_largeur, 9 * unite_hauteur);
            graphic.DrawString("Désignation".ToUpper(), fnt33, Brushes.White, 2 * unite_largeur, 9 * unite_hauteur);
            graphic.DrawString("px unit".ToUpper(), fnt33, Brushes.White, 10 * unite_largeur + 18, 9 * unite_hauteur);
            graphic.DrawString("Qté".ToUpper(), fnt33, Brushes.White, 13 * unite_largeur - 3, 9 * unite_hauteur);
            graphic.DrawString("Px total".ToUpper(), fnt33, Brushes.White, 14 * unite_largeur + 10, 9 * unite_hauteur);
            ...
            for (int i = 0; i <= dgvVente.Rows.Count - 1; i++)
            {
                int Yloc = 10 * unite_hauteur + 10 + i * unite_hauteur;
                graphic.DrawString((i + 1).ToString(), fnt1, Brushes.Black, unite_largeur, Yloc);
                if (dgvVente.Rows[i].Cells[1].Value.ToString().Length > 24)
                {
                    graphic.DrawString(dgvVente.Rows[i].Cells[1].Value.ToString().Substring(0, 24).ToUpper() + "...", fnt1, Brushes.Black, 2 * unite_largeur, Yloc);
                }
                else
                {
                    graphic.DrawString(dgvVente.Rows[i].Cells[1].Value.ToString().ToUpper(), fnt1, Brushes.Black, 2 * unite_largeur, Yloc);
                }
                graphic.DrawString(dgvVente.Rows[i].Cells[2].Value.ToString(), fnt1, Brushes.Black, 10 * unite_largeur + 18, Yloc);
                graphic.DrawString(dgvVente.Rows[i].Cells[5].Value.ToString(), fnt1, Brushes.Black, 13 * unite_largeur - 3, Yloc);
                var total = Decimal.Parse(dgvVente.Rows[i].Cells[5].Value.ToString()) * Decimal.Parse(dgvVente.Rows[i].Cells[2].Value.ToString());
                graphic.DrawString(total.ToString(), fnt1, Brushes.Black, 14 * unite_largeur + 10, Yloc);

                montantTotal += total;
                totalRemise += Decimal.Parse(dgvVente.Rows[i].Cells[5].Value.ToString()) *
                    (Decimal.Parse(dgvVente.Rows[i].Cells[2].Value.ToString()) - Decimal.Parse(dgvVente.Rows[i].Cells[3].Value.ToString()));
            }
            Loc += 8 * unite_hauteur + 20 + detail_hauteur_facture;
            graphic.FillRectangle(Brushes.SteelBlue, 25, Loc, 16 * unite_largeur, 25);
            graphic.DrawString("Total ", fnt33, Brushes.White, unite_largeur, Loc + 3);

            if (totalRemise > 0)
            {
                var taux = totalRemise * 100 / montantTotal;
                var netPayer = montantTotal - totalRemise;
                graphic.DrawString(montantTotal + "F.CFA        -        Net à payer :  " + netPayer + " F.CFA", fnt33, Brushes.White, 6 * unite_largeur - 10, Loc + 3);
                graphic.DrawString("Total remise (" + taux + "%)  :  " + totalRemise.ToString() + " F.CFA", fnt33, Brushes.Black, 9 * unite_largeur, Loc + unite_hauteur + 10);
            }
            else
            {
                graphic.DrawString(montantTotal.ToString() + " FCFA", fnt33, Brushes.White, 13 * unite_largeur, Loc + 3);
            }
            graphic.DrawString(typeVente + DateTime.Now.ToShortDateString(), fnt1, Brushes.Black, 25, Loc + 2 * unite_hauteur + 10);
            graphic.DrawString("Merci pour votre visite ", fnt1, Brushes.Black, 25, Loc + 3 * unite_hauteur + 10);
            graphic.DrawString("Bonne guerison ", fnt1, Brushes.Black, 25, Loc + 4 * unite_hauteur + 10);
