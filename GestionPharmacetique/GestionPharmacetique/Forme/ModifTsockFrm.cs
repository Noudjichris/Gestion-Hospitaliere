...
Graphics mGraphics = e.Graphics;
        ...
        private void ModifTsockFrm_Load(object sender, EventArgs e)
        {
            ...
            lblMontantHT.Width = dgvProduit.Width / 6 +93;
                        ...
                        if (Int32.TryParse(txtStock.Text, out stock) && decimal.TryParse(lblPrixAchat.Text, out prix))
                        {
                            ...
                            if (!string.IsNullOrEmpty(numeroProduit))
                            {
                                ...
                                if (dgvProduit.Rows.Count > 0)
                                {
                                    ...
                                    {
                                        if (produit.NumeroProduit == dgvProduit.Rows[i].Cells[0].Value.ToString() && indexRow !="1" )
                                        {
            ...
            if (e.ColumnIndex ==7)
            {
                try
                {
                    if (dgvProduit.Rows.Count > 0)
                    {
                        ...
                        {
                            var p = new Produit();
                            p.DifferenceStock = Convert.ToInt32(dgvProduit.SelectedRows[0].Cells[4].Value.ToString());
