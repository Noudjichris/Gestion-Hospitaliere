...
namespace GestionDuneClinique.Formes
{
    public partial class ConsultationFrm : Form
    {
        ...
        void ListeConsultation(List<Consultation> listeConsult)
        {
            try
            {
                ...
                foreach (Consultation consultation in listeConsult)
                {
                    ...
                    //{
                    flag = AppCode.ConnectionClassClinique.SiFactureEnBon("CONSULTATION", consultation.NumeroConsultation);
                    if (flag)
                    {
                     dgvConsult.Rows[dgvConsult.Rows.Count-1].DefaultCellStyle.BackColor = Color.White;
                    }
                    //}
                }
            }
                          ...
                          var consultation= new Consultation();
                            ...
                            if (etat == "1")
                            {
                                ...
                                if (ConnectionClassClinique.EnregistrerUneConsultation(consultation))
                                {
                                    ...
                                    {
                                        consultation .NumeroConsultation= ConnectionClassClinique.ObtenirDerniereConsultation();
                                        ConnectionClassClinique.BondeConsultation(consultation);
                                    }
                                    if (State == "1")
                                    {
            ...
            try
            {
                if (dgvConsult.SelectedRows.Count > 0)
                {
                    ...
                    foreach (var p in listePatient)
                        patient = p;
                    bitmap = Impression.FactureConsultation(idConsul,dtpConsult.Value.Date, dgvConsult, patient, GestionAcademique.LoginFrm.nom);
                    ...
                    sfd.FileName = "Rapport de " + patient.Nom + " " + patient.NumeroPatient + " _imprimé_le_" + date + ".pdf";
                    ...
                    if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        pageIndex.addImage(img1, 0,-25, pageIndex.height, pageIndex.width);
                        //if (rowCount > 8)
                        //{
                        ...
                        //            analyseBitmap = Impression.FactureOfficielle(dataGridView1, remise, nomEmploye, i);
                        //            inputImage = @"cdali" + i;
                        //            // Create an empty page
                        //            pageIndex = document.addPage(330, 500);

                        //            document.addImageReference(analyseBitmap, inputImage);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawImage(bitmap, -5, 20, 907,530);
                                ...
                                bool.Parse(dataGridView2.SelectedRows[0].Cells[5].Value.ToString()))
                            {
                                ...
                                consultation.Partenaire = "";
                                {
                                    if (ConnectionClassClinique.BondeConsultation(consultation))
                                    {
                                        ...
                                        btnEnregistrer.Enabled = false;
                                        btnAjouter.Enabled = true;
                                        var listeConsult = ConnectionClassClinique.ListeDesConsultations(consultation.DateConsultation.Date);
                                        ListeConsultation(listeConsult);
                                    }
                                }
                            }
