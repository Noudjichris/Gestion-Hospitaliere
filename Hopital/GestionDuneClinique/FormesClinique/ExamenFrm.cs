            ...
            try
            {
                ...
                if (dgvAnal.Rows.Count > 0)
                {
                    ...
                    foreach (var p in listePatient)
                        patient = p;
                    analyseBitmap = Impression.FactureOfficiellePourBonDesActes(idAnalyse, dgvAnal, patient, dateTimePicker1.Value, GestionAcademique.LoginFrm.nom);
                        ...
                        montantTotal = Double.Parse(txtmontanttotal.Text);
                             ...
                             if (etat == "1")
                            {
                                ...
                                if (ConnectionClassClinique.AjouterLesAnalysesEffectues(analyse, dgvAnal, nomPatient))
                                {
                                    ...
                                    analyse.NumeroAnalyse = idExamen;
...
printPreviewDialog1.Document = printDocument1;
                   ...
                   dataGridView2.SelectedRows[0].Cells[2].Value.ToString();
                                   ...
                                   bool.Parse(dataGridView2.SelectedRows[0].Cells[5].Value.ToString()))
                    {
                        ...
                        if (ConnectionClassClinique.BondesExamens(analyse, dgvAnal))
                        {
                            ...
                            foreach (var p in listePatient)
                                patient = p;
                            analyseBitmap = Impression.FactureOfficiellePourBonDesActes(idExamen, dgvAnal, patient, dateTimePicker1.Value, GestionAcademique.LoginFrm.nom);
        ...
        private void textBox2_TextChanged_1(object sender, EventArgs e)
        {
            ...
            var listeEntrep =from en in  ConnectionClassClinique.ListeDesEntreprises()
                                 where en.NomEntreprise.ToUpper().Contains(textBox2.Text.ToUpper())
                                 select en;
            cmbEntreprise.Items.Add("");
            foreach (Entreprise entreprise in listeEntrep)
            {
