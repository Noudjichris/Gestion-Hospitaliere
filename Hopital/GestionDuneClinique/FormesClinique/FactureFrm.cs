...
namespace GestionDuneClinique.Formes
{
    public partial class FactureFrm : Form
    {
            ...
            try
            {
                ...
                if (MonMessageBox.ShowBox("Voulez vous ajouter de nouveau analyse?", "Confirmation", "confirmation.png") == "1")
                {
                    ...
                    if (GestionPharmacetique.Forme.ExamenFrm.ShowBox() == "1")
                    {
                        ...
                        flagConven = GestionPharmacetique.Forme.ExamenFrm.flagConv;
                    }
                }
                else
                {
            ...
            try
            {
                ...
                if (MonMessageBox.ShowBox("Voulez vous ajouter une nouvelle consultation?", "Consultation", "consultation.png") == "1")
                {
                    ...
                    if (ConsultationFrm.ShowBox() == "1")
                    {
                        ...
                        flagConven = ConsultationFrm.flagConv;
            ...
            try
            {
                if (txtPatient.Text !="")
                {
                        if (Double.TryParse(txtPaye.Text, out montantPaye))
                        {
                            ...
                            facture.Reste = reste;
                            var siConventionne = checkBox2.Checked;
                            if (etat == "1")
                            {
                                if (ConnectionClassClinique.EnregistrerUneFacture(facture, dataGridView1, numeroActe, typeActe, montantPaye, siConventionne))
                                {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawImage(factureBitmap, 0, 10, 500, 300);
        ...
        private void btnImprimer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    double reste = 0;
                    if (Double.TryParse(txtReste.Text, out reste))
                    { }

                    factureBitmap = Impression.FactureOfficielle(numFacture, dataGridView1, patient, modePaiement, remise, reste , nomEmploye);
                    ...
                    pageIndex.addImage(img1, -0, -33, pageIndex.height, pageIndex.width);
                    if (rowCount > 8)
                    {
        ...
        private void txtPaye_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                txtPaye.BackColor = Color.White;
                double montantPaye = 0, total, dejaPaye, remise = 0.0;
                if (Double.TryParse(txtPaye.Text, out montantPaye))
                {
                }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Rows.Count > 0)
                {
                    ...
                    if (!found)
                    {
                        dataGridView1.Rows.Add("CARNET", frais, "1", frais, "Consultation");
                    }
                }
                else
                {
        ...
        private void checkBox2_CheckedChanged_1(object sender, EventArgs e)
        {
            try
            {
                if (idPatient > 0)
                {
                    if (!string.IsNullOrWhiteSpace(ConnectionClassClinique.ListeDesPatients(idPatient)[0].NomEntreprise))
                    {
