    ...
    public partial class RapportMedecinFrm : Form
    {
        ...
        void ListeDesConsultation()
        {
            try
            {
              ...
              " WHERE pro_det_fact.groupage = 'Consultation' AND patient_tbl.entrep NOT LIKE '' AND pro_det_fact.design=@designation group by  pro_det_fact.design";
                        var dtConsultationPro = AppCode.ConnectionClassClinique.TableFacture(requete, dtp1.Value.Date, dtp2.Value.Date.AddHours(24), dtConsultation.Rows[i].ItemArray[0].ToString());
        ...
        private void ListeDesRapportsVentesMedicaments()
        {
            try
            {
                ...
                if (cmbConventionne.Text == "<<TOUS LES PARTICULIERS>>")
                {
                    dtPharmacie = ConnectionClassPharmacie.ListeDesVentesDataTableParticulier(dtp1.Value.Date, dtp2.Value.Date);
                    Montant = ConnectionClassPharmacie.ListeDesVentes(dtp1.Value.Date, dtp2.Value.Date.AddHours(24));
                ...
                else if (cmbConventionne.Text == "<<TOUS LES PATIENTS>>" || string.IsNullOrWhiteSpace(cmbConventionne.Text))
                {
                    ...
                    Montant = ConnectionClassPharmacie.ListeDesVentes(dtp1.Value.Date, dtp2.Value.Date.AddHours(24));
                    dtPharmacie = ConnectionClassPharmacie.ListeDesVentesDeTousLesConventionnes(dtp1.Value.Date, dtp2.Value.Date);
                    for (int i = 0; i < dtPharmacie.Rows.Count; i++)
                    {
                        Montant += double.Parse(dtPharmacie.Rows[i].ItemArray[1].ToString());
                    }
                ...
                {
                    dtPharmacie = ConnectionClassPharmacie.ListeDesVentesPharmacieParConvention(cmbConventionne.Text, dtp1.Value.Date, dtp2.Value.Date); dataGridView3.Rows.Clear();
                    for (int i = 0; i < dtPharmacie.Rows.Count; i++)
                    {
                        var total = double.Parse(dtPharmacie.Rows[i].ItemArray[1].ToString()) *
                            Int32.Parse(dtPharmacie.Rows[i].ItemArray[2].ToString());
