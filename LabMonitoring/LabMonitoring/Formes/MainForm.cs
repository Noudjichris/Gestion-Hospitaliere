        ...
        private void txtGroupeExam_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var liste = from lr in ConnectionClass.ListeResultatPatient()
                            join lp in ClassClinique.ListeDesPatients()
                            ...
                            where lr.DateResultat >= DateTime.Now.Date.AddDays(-60)
                           orderby lr.DateResultat descending
                            select new
                            {
                                lp.NomPatient,lp.IDPatient,lr.NumeroResultat, lr.DateResultat
                            };
                foreach (var l in liste)
                {
                    dgvGroupe.Rows.Add(l.NumeroResultat,l.IDPatient,l.NomPatient,l.DateResultat);
                }
            }
            catch (Exception )
            {
