...
namespace GestionDesPelerinsTchad.Formes
{
    public partial class ModifierMotPasse : Form
    {
        ...
        {
            var utilisateur = LabMonitoring.AppCode.ClassClinique.ListesDesUtilisateurs(numeroUtilisateur, textBox4.Text.GetHashCode().ToString());
            ...
            try
            {
                if (!string.IsNullOrEmpty(textBox1.Text))
                {
                    if (textBox3.Text.Length >= 6)
                    {
                        ...
                        {
                            var utilisateur = new LabMonitoring.AppCode.Utilisateur(numeroUtilisateur, textBox1.Text,
                                textBox3.Text.GetHashCode().ToString(), LoginFrm.typeUtilisateur);
                            if (LabMonitoring.AppCode.ClassClinique.ModifierUnUtilisateur(utilisateur))
                            {
                                Close();
                            }
                        } else
                        {
                            GestionPharmacetique.MonMessageBox.ShowBox("Les mots de passe ne sont pas conformes", "Erreur", "erreur.png");
                        }
                    } else
                    {
                        GestionPharmacetique.MonMessageBox.ShowBox("Le mot de passe doit comporter au moins six(6) caracteres", "Erreur", "erreur.png");
                    }
                }
                else
                {
                    GestionPharmacetique.MonMessageBox.ShowBox("Veuillez saisir le nom d'utilisateur", "Erreur", "erreur.png");
                }
            }
            catch (Exception ex )
            {
                GestionPharmacetique.MonMessageBox.ShowBox("Modifier utilisateur", ex);
            }
        }
