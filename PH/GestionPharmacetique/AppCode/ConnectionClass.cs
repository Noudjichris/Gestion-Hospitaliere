...
namespace GestionPharmacetique.AppCode
{
    public class ConnectionClass
    {
        ...
        {
             connectionString =
                 @"server=192.168.1.1;user id=Al-shifa;password=Al-shifa2021;database=pharmdb"; ;
                 //@"server=localhost;user id=root;password=legacy2019;database=pharmdb";
        ...
        public static List<Employe> ListeDesEmployees()
        {
            ...
            try
            {
                string requete = "SELECT * FROM employe ORDER BY num_empl ";
                connection.Open();
                command.CommandText = requete;
                MySqlDataReader reader = command.ExecuteReader();
            ...
            try
            {
                ...
                else
                {
                    if (MonMessageBox.ShowBox("Voulez vous modifier les données de cet employé?", "Confirmation", "confirmation.png") == "1")
                    {
                        ...
                        command.CommandText = requete;
                        command.ExecuteNonQuery();
                        MonMessageBox.ShowBox("données d'employé modifiées avec succés.", "Information modification",
                           "affirmation.png");
                    }
                }
            }
        ...
        public static bool CreerUneLivraison(Livraison livraison)
        {
            try
            {
                ...
                command.Parameters.Add(new MySqlParameter("num_bll", livraison.NumLivraison));
                command.CommandText = requete;
                var reader = command.ExecuteReader();
                if (reader.HasRows)
                {
                    reader.Close();
                    MonMessageBox.ShowBox("livraison numéro " + livraison.NumLivraison + " existe deja dans la base de données",
                ...
                {
                    reader.Close();
                    requete = string.Format("INSERT INTO `detail_bl` ( `num_bl`,`date_bl`, `num_fourn`,autre) " +
                                            " VALUES ( @num_bl, @date_bl, @num_fourn, @autre)");
                    ...
                    command.Parameters.Add(new MySqlParameter("num_bl", livraison.NumLivraison));
                ...
                MonMessageBox.ShowBox("livraison numéro " + livraison.NumLivraison + " creéé avec succés",
                    "Information Insertion", "affirmation.png");
                return true;
            }
            catch (Exception exception)
            ...
            string etat, int rowIndex)
        {
            ...
            try
            {
                ...
                if (etat == "1")
                {
                    requete = "SELECT COUNT(*) FROM bn_liv WHERE num_bl = '"+livraison.NumLivraison+"'";
                    command.CommandText = requete;
                    ...
                    {
                        livraison.ID=Int32.Parse(dt.Rows[i].ItemArray[0].ToString())+1;
                    }
                    requete = string.Format("INSERT INTO `bn_liv` (`num_bl`, `num_medi`, `prix_achat`,`quantite`,`prix_total`,id) VALUES ('{0}','{1}', {2}, {3},{4},{5})",
                        livraison.NumLivraison, livraison.NumProduit, livraison.PrixAchat, livraison.Quantite, livraison.PrixAchat * livraison.Quantite,livraison.ID);
                   
                    command.CommandText = requete;
                    command.Transaction = _transaction;
                    command.ExecuteNonQuery();

                    requete = string.Format(@"UPDATE stock SET grd_stock =grd_stock + {0} WHERE num_medi = '{1}'", livraison.Quantite, livraison.NumProduit);
                ...
                {
                    var AncienneQuantite = Convert.ToDouble(dgvLivraison.Rows[rowIndex].Cells[5].Value.ToString());//quantite inserer avant modification
                    livraison.ID = Convert.ToInt32(dgvLivraison.Rows[rowIndex].Cells[0].Value.ToString());

                    
                    requete = string.Format("UPDATE `bn_liv` SET  `prix_achat`={0},`quantite`={1},`prix_total`={2} WHERE "+
                        "id = {3} AND num_medi=@num_medic AND num_bl=@num_livr",
                    livraison.PrixAchat, livraison.Quantite, livraison.PrixAchat * livraison.Quantite, livraison.ID);
                   command.Parameters.Add(new MySqlParameter("num_medic",livraison.NumProduit));
                   command.Parameters.Add(new MySqlParameter("num_livr", livraison.NumLivraison));
                    command.CommandText = requete;
                    command.Transaction = _transaction;
                    command.ExecuteNonQuery();
                    ...
                    dgvLivraison.Rows[rowIndex].Cells[5].Value = String.Format(elGR, "{0:0,0}", livraison.Quantite);
                    dgvLivraison.Rows[rowIndex].Cells[6].Value = String.Format(elGR, "{0:0,0}", livraison.Quantite * livraison.PrixAchat);

                    requete = string.Format(@"UPDATE stock SET grd_stock =grd_stock - {0} +{1} WHERE(num_medi = '{2}')", AncienneQuantite, livraison.Quantite, livraison.NumProduit);
                    command.CommandText = requete;
                    command.Transaction = _transaction;
                    command.ExecuteNonQuery();
                }
        ...
        public static void SupprimerLivraison(string numeroLivraison)
        {
            ...
            {
                connection.Open();
                string requete = "SELECT * FROM bn_liv WHERE num_bl = '" + numeroLivraison + "'";
                command.CommandText = requete;
                MySqlDataReader reader = command.ExecuteReader();
                DataTable dataTable = new DataTable();
                dataTable.Load(reader);
                ...
                if (dataTable.Rows.Count > 0)
                {
                    ...
                    var btnClick =
                        MonMessageBox.ShowBox(
                            "Voulez vous mettre automatiquement à jour les quantités des produits supprimés?",
                            "Demande de confirmation", "confirmation.png");
                    if (btnClick == "1")
                    {
                        for (int I = 0; I <= dataTable.Rows.Count - 1; I++)
                        {
                            string numMedicament = dataTable.Rows[I].ItemArray[1].ToString();
                            int quantite = Int32.Parse(dataTable.Rows[I].ItemArray[2].ToString());

                            // mettre a jour les quantites des dans le stock
                            requete =
                                string.Format(
                                    "UPDATE `stock` SET `quantite` = (`quantite` - {0} ) WHERE `num_medi` = ('{1}')",
                                    quantite, numMedicament);
                            command.CommandText = requete;
                            _modifier = command.ExecuteNonQuery();

                        }
                        if (_modifier > 0)
                        {
                            MonMessageBox.ShowBox("Les données de stocks ont été mises à jour avec succés",
                                "Information suppression", "affirmation.png");
                        }
                    }
                }
        ...
        public static void RetirerLivraison(int id, string numProduit, int quantite, string numLivraison)
        {
            try
            {
                connection.Open();
                var requete = string.Format("DELETE FROM bn_liv WHERE id = {0} AND num_medi=@num_medic AND num_bl=@num_livr",id );
                   command.Parameters.Add(new MySqlParameter("num_medic",numProduit));
                   command.Parameters.Add(new MySqlParameter("num_livr", numLivraison));
                command.CommandText = requete;
                command.ExecuteNonQuery();
                ...
                command.CommandText = requete;
                command.ExecuteNonQuery();
                MonMessageBox.ShowBox("Données retirées avec succés. Et la quantité a été mise à jour"
, "Information suppression", "affirmation.png");
            }
            catch (Exception exception)
            {
        ...
        public static ArrayList ListeDesLivraisons(string numeroLivraison)
        {
            ArrayList listeLivraison = new ArrayList();
            try
            {
                string requete = "SELECT detail_bl.num_bl, detail_bl.date_bl,detail_bl.num_fourn, fournisseur.nom_fourn " +
                                 ",  detail_bl.montant_fact,detail_bl.autre FROM (detail_bl INNER JOIN  fournisseur ON " +
                ...
                while (reader.Read())
                {
                    DateTime dateLiv = reader.GetDateTime(1);
                    decimal montant = !reader.IsDBNull(4) ? reader.GetDecimal(4) : 0m;
                    decimal autres = !reader.IsDBNull(5) ? reader.GetDecimal(5) : 0m;
                    int numFourn = reader.GetInt32(2);
                    string nomFourn = reader.GetString(3);
                    Livraison livraison = new Livraison(numeroLivraison, dateLiv, numFourn, nomFourn, autres, montant);
        ...
        //liste de la livraison
        public static ArrayList ListeDesDetailsLivraisons(string numeroLivraison)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    var codeMedi = reader.GetString(1);
                    var qte = reader.GetInt32(2);
                    var prixAchat = reader.GetDecimal(3);
                    var prixVente = reader.GetDecimal(4);
                    var nomMedi = reader.GetString(5);
                    var livraison = new Livraison(id, numeroLivraison, codeMedi, nomMedi, prixAchat, prixVente, qte);
                    listeLivraison.Add(livraison);
                }
            }
            finally
        ...
        public static DataTable  ListeDesDetailsLivraisons(string designation, DateTime date1,  DateTime date2)
        {
            ...
            try
            {
                var requete = "SELECT bn_liv.num_bl, bn_liv.num_medi, bn_liv.quantite, bn_liv.prix_achat," +
                                 " medicament.prix_vente, medicament.nom_medi,bn_liv.id FROM bn_liv INNER JOIN medicament ON bn_liv.num_medi" +
                                 " = medicament.num_medi INNER JOIN detail_bl ON bn_liv.num_bl=detail_bl.num_bl WHERE medicament.nom_medi =@nom AND detail_bl.date_bl >=@date1 AND detail_bl.date_bl<@date2";
                command.CommandText = requete;
                connection.Open();
        ...
        public static ArrayList ListeDesLivraisonsParFournisseur(string fournisseur)
        {
            ...
            try
            {
                var requete = "SELECT detail_bl.num_bl, detail_bl.date_bl,  detail_bl.num_fourn, fournisseur.nom_fourn , detail_bl.montant_fact," +
                                 " detail_bl.autre FROM (detail_bl INNER JOIN  fournisseur ON detail_bl.num_fourn = fournisseur.num_fourn)" +
                                 " WHERE fournisseur.nom_fourn = '" + fournisseur + "' ORDER BY detail_bl.date_bl DESC";
                command.CommandText = requete;
                connection.Open();
                var reader = command.ExecuteReader();
                while (reader.Read())
                {
                    var num_liv = reader.GetString(0);
                    var dateLiv = reader.GetDateTime(1);
                    var numFourn = reader.GetInt32(2);
                    var nomFourn = reader.GetString(3);
                    decimal montant = !reader.IsDBNull(4) ? reader.GetDecimal(4) : 0m;
                    decimal autres = !reader.IsDBNull(5) ? reader.GetDecimal(5) : 0m;

                    Livraison livraison = new Livraison(num_liv, dateLiv, numFourn, nomFourn, autres, montant);
        ...
        public static bool MettreAJourUneLivraison(Livraison livraison)
        {
            try
            {
                ...
                var requete = string.Format(@"UPDATE detail_bl SET autre = {0} WHERE(num_bl = '{1}')",
                    livraison.AutresCharges, livraison.NumLivraison);
                command.Transaction = _transaction;
                command.CommandText = requete;
                command.ExecuteNonQuery();
                return true;
        ...
        public static bool InsererLeRetour(string id_produit, string medic, double prix, int qte, string motif, string numLivr)
        {
            bool flag = false;
            try
            {
        ...
        public static List<Medicament> ListeDesMedicamentsRechercherParNom(string nomMedicament)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    produit .Description = reader.GetString(6);
        ...
        public static List<Medicament> ListeDesMedicamentsExpirees()
        {
            ...
            try
            {
                ...
                var dateExpiration = DateTime.Now.Date.AddMonths(2);
        ...
        public static List<Medicament> ListeDesMedicamentsAStockBas()
        {
            ...
            try
            {
                string requete = "SELECT medicament.num_medi, medicament.numeroFamille, medicament.nom_medi, " +
                                 " medicament.prix_achat, medicament.prix_vente,medicament.qteAlerte, medicament.description, " +
                                 "medicament.date_expiration, stock.quantite FROM (medicament INNER JOIN " +
                         " stock ON medicament.num_medi = stock.num_medi AND stock.quantite <= medicament.qteAlerte ) ORDER BY" +
                " medicament.nom_medi";
                command.CommandText = requete;
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
    ...
    "Information modification", "affirmation.png");
        ...
        public static List<Produit> LogStock(int etat)
        {
            try
            {
                var listeProduit = new List<Produit>();
               var  _requete = "SELECT * FROM stock_mod WHERE etat= " + etat;
                ...
                while (reader.Read())
                {
                    ...
                    produit.DateModification = reader.GetDateTime(2);
                    produit.IDUser = !reader.IsDBNull(3) ? reader.GetString(3) : ""; //reader.GetString(3);
                    produit.Motif = !reader.IsDBNull(4) ? reader.GetString(4) : "";
                    produit.PrixAchat = reader.GetDouble(5);
                    produit.DifferenceStock = reader.GetInt32(6);
                    produit.NombreEntree = !reader.IsDBNull(7) ? reader.GetInt32(7) : 0;
                    produit.NombreSortie = !reader.IsDBNull(8) ? reader.GetInt32(8) : 0;
        ...
        //creer un produit
        public static bool ModifierStock(Produit produit, int etat, string reference)
        {
            try
            {
                connection.Open();
                _transaction = connection.BeginTransaction();
               var  _requete = "INSERT stock_mod (id_prod, date, id_user,motif, qte,prix,etat,idRef) VALUES(@id_prod, @date, @id_user,@motif,@qte,@prix,@etat,@idRef)";
                command.Parameters.Add(new MySqlParameter("id_prod", produit.NumeroProduit));
                command.Parameters.Add(new MySqlParameter("date", produit.DateModification));
                command.Parameters.Add(new MySqlParameter("id_user", produit.IDUser));
                command.Parameters.Add(new MySqlParameter("motif", produit.Motif));
                ...
                command.Parameters.Add(new MySqlParameter("idRef", produit.NombreSortie));
                command.CommandText = _requete;
                command.Transaction = _transaction;
                command.ExecuteNonQuery();
                ...
                if (etat == 1)
                {
                    if (reference.ToUpper() == "Mouvement du grand stock vers la pharmacie".ToUpper())
                    {
                ...
                {
                    if (reference.ToUpper() == "Mouvement du grand stock vers le laboratoire".ToUpper()
                        || reference.ToUpper() == "Mouvement du grand stock vers la salle de soin".ToUpper()
                                    || reference.ToUpper().Contains("Mouvement du grand stock".ToUpper()))
                    {
        ...
        public static bool RetirerStock(Produit produit, int etat, string reference)
        {
            try
            {
                connection.Open();
                _transaction = connection.BeginTransaction();
               var  _requete = "DELETE FROM stock_mod WHERE id = " + produit.DifferenceStock;
                command.CommandText = _requete;
                command.Transaction = _transaction;
                command.ExecuteNonQuery();

                _requete = "SELECT * FROM stock WHERE num_medi = " + produit.NumeroProduit;
                command.CommandText = _requete;
                command.Transaction = _transaction;
                var reader = command.ExecuteReader();
                var dt = new DataTable();
                ...
                if (dt.Rows.Count > 0)
                {
                    stock = Convert.ToInt32(dt.Rows[0].ItemArray[2].ToString());
                }
                if (etat == 1)
                {
                    ...
                    {
                        _requete = string.Format("UPDATE stock SET quantite =quantite - {0}, grd_stock =grd_stock+{0} WHERE num_medi ='{1}'",
                              produit.Stock, produit.NumeroProduit);
                    }
                    else
                    {
                        _requete = string.Format("UPDATE stock SET quantite =quantite - {0} WHERE num_medi ='{1}'",
        ...
        public static bool MettreAJourModificationStock(Produit produit, int ancienQte, int etat,string reference)
        {
            try
            {
                connection.Open();
                _transaction = connection.BeginTransaction();
                var _requete = "UPDATE stock_mod SET date=@date, id_user=@id_user,motif=@, qte=@qte,prix=@prix WHERE id_prod=@id_prod AND id = @id";
                command.Parameters.Add(new MySqlParameter("id", produit.NombreJour));
                command.Parameters.Add(new MySqlParameter("id_prod", produit.NumeroProduit));
                command.Parameters.Add(new MySqlParameter("date", produit.DateModification));
                command.Parameters.Add(new MySqlParameter("id_user", produit.IDUser));
                command.Parameters.Add(new MySqlParameter("motif", produit.Motif));
                ...
                if (dt.Rows.Count > 0)
                {
                    stock1 = Convert.ToInt32(dt.Rows[0].ItemArray[3].ToString());
                }
                if (etat == 1)
                {
                    if (reference.ToUpper() == "Mouvement du grand stock vers la pharmacie".ToUpper())
                    {
                        _requete = string.Format("UPDATE stock SET quantite =quantite-{0} +{1}, grd_stock=+{0}-{1} WHERE num_medi ='{2}'",
                            ancienQte, produit.DifferenceStock, produit.NumeroProduit);
                    }
                    else
                    {
                        _requete = string.Format("UPDATE stock SET quantite =quantite-{0} +{1} WHERE num_medi ='{2}'",
                            ancienQte, produit.DifferenceStock, produit.NumeroProduit);
                    }
                }
                else if (etat == 2)
                {
                    var stock = produit.DifferenceStock - ancienQte;
                    if (stock1 < stock)
                    {
                        stock = 0;
                    }
          ...
          public static bool CreerUnNouveauDocumentStock(DocumentStock doc)
        {
            try
            {
                connection.Open();
                //_transaction = connection.BeginTransaction();
               var  _requete = "INSERT INTO doc_stock (reference,date,source,destin) value (@reference,@date,@source,@destin)";
        ...
        public static bool MettreAjourDocumentStock(DocumentStock doc)
        {
            ...
            {
                connection.Open();
                var _requete = "UPDATE doc_stock SET reference=@reference,date=@date,source=@source,destin= @destin WHERE id =" + doc.ID;
        ...
        public static List<DocumentStock> ListeDocumentStock()
        {
            try
            {
                var listeProduit = new List<DocumentStock>();
                var _requete = "SELECT * FROM doc_stock ";
                command.CommandText = _requete;
                connection.Open();
                var reader = command.ExecuteReader();
                while (reader.Read())
        ...
        // enregistrer une vente
        public static bool EnregistrerVente(Vente vente, DataGridView dgvVente, Client client)
        {
            bool flag = false;
            try
            {
                ...
                string requete = "SELECT * FROM clienttbl WHERE nomClient =  '" + client.NomClient + "' AND matricule = '"+client.Matricule+"'";
                command.CommandText = requete;
                command.Transaction = _transaction;
                ...
                if (dt.Rows.Count > 0)
                {
                    client.Id = Int32.Parse(dt.Rows[0].ItemArray[0].ToString());
                }
                else
                {
                ...
                command.Parameters.Add(new MySqlParameter("prix_total", vente.PrixTotal));
                ...
                command.Parameters.Add(new MySqlParameter("etat", vente.SiVente));
                command.CommandText = requete;
                command.Transaction = _transaction;
                command.ExecuteNonQuery();
                ...
                if (vente.SiVente)
                {
                    ...
                    command.Parameters.Add(new MySqlParameter("montant", vente.PrixTotal));
                ...
                for (int I = 0; I <= dgvVente.Rows.Count - 1; I++)
                {
                    ...
                    var indexDetail =dgvVente.Rows[I].Cells[7].Value.ToString();
                    ...
                    {
                        requete =
                              string.Format("UPDATE `medicament` SET `NbreDetail` = (`NbreDetail` - {0} ) WHERE `num_medi` = ('{1}')",
                                  quantite, numMedicament);
                        command.CommandText = requete;
                        command.Transaction = _transaction;
                        command.ExecuteNonQuery();
                        dt.Dispose();
                        requete =
                              string.Format("SELECT * FROM `medicament` WHERE `num_medi` = ('{0}')", numMedicament);
                        command.CommandText = requete;
                        command.Transaction = _transaction;
                        var reader1 = command.ExecuteReader();
                        var dt1 = new DataTable();
                        ...
                        if (nbreDetaille <= 0)
                        {
                            dt1.Dispose();
                            requete =
                              string.Format("SELECT * FROM `stock` WHERE `num_medi` = ('{0}')", numMedicament);
                            command.CommandText = requete;
                            command.Transaction = _transaction;
                            var reader3 = command.ExecuteReader();
                            var dt3 = new DataTable();
                            dt3.Load(reader3);
                            var stock = Int32.Parse(dt3.Rows[0].ItemArray[2].ToString());
                            ...
                            {
                                requete =
                               string.Format("UPDATE `stock` SET `quantite` = (`quantite` - 1 ) WHERE `num_medi` = ('{0}')",
                                 numMedicament);
                                command.CommandText = requete;
                                command.Transaction = _transaction;
                                command.ExecuteNonQuery();
                                ...
                                requete =
                              string.Format("UPDATE `medicament` SET `nbreDetail` = (`nbreDetail` + {0} ) WHERE `num_medi` = ('{1}')", nbreEnBoite
                                , numMedicament);
                                command.CommandText = requete;
                                command.Transaction = _transaction;
                                command.ExecuteNonQuery();
        ...
        public static DataTable ListeDesVentesParDateVente(int numeroVente)
        {
            ...
            try
            {
                string requete = "SELECT  vente.num_medi,medicament.nom_medi, vente.prix_achat, vente.prix_vente, " +
                           ...
                           " WHERE vente.num_vente =  " + numeroVente +" GROUP BY vente.prix_achat";
                connection.Open();
                command.CommandText = requete;
                MySqlDataReader reader = command.ExecuteReader();
                dtVente.Load(reader);
       ...
       #region Encaissement
        ...
        public static DataTable RapportStatistiqueParMontant(DateTime dateDebut, DateTime dateFin)
        {
            DataTable dt = new DataTable();
            try
            {
                ...
                string query = "SELECT medicament.nom_medi, vente.prix_vente, SUM(vente.quantite), SUM(vente.prixTotal), " +
                    " detail_vente.date_vente, medicament.num_medi FROM vente INNER JOIN medicament ON vente.num_medi = medicament.num_medi INNER " +
                    " JOIN detail_vente ON vente.num_vente = detail_vente.num_vente GROUP BY medicament.nom_medi, medicament.num_medi HAVING " +
        ...
        public static int EnregistrerInventaire( List<Medicament> listeProduit)
        {
            var numProduit="";
            try
            {
                connection.Open();
                _transaction = connection.BeginTransaction();
                var requete = "INSERT INTO invent_tbl (`date`) VALUES(@date)";
                command.Parameters.Add(new MySqlParameter("date", DateTime.Now));
                command.Transaction = _transaction;
                command.CommandText = requete;
                command.ExecuteNonQuery();
                ...
                {
                    var detIndex=0;
                    requete = "INSERT INTO det_invent (id, id_prod,qte, pc, pp,empla,ordre,qte_av,det)" +
                            "VALUES(@idd, @id_prod,@qte,@pc,@pp, @empl,@ordre,@qte_av," + detIndex +")";
                    var qte = 0;
                    ...
                    command.Parameters.Add(new MySqlParameter("qte_av", produit.Quantite));
                    command.Transaction = _transaction;
                    command.CommandText = requete;
                    command.ExecuteNonQuery();
                    if (produit.NombreBoite > 0 && produit.PrixVenteDetail > 0)
                    {
                        detIndex = 1;
                        requete = "INSERT INTO det_invent (id, id_prod,qte, pc, pp,empla,ordre,qte_av,det)" +
                               "VALUES(@idd1, @id_prod1,@qte1,@pc1,@pp1, @empl1,@ordre1,@qte_av1," + detIndex + ")";
                        j++;
                        numProduit = produit.NumeroMedicament;
        ...
        public static bool EnregistrerInventaire(Inventaire inventaire) //DataGridView dgvProduit, int rowIndex)
        {
            try
            {
                ...
                var requete = "SELECT * FROM det_invent WHERE id = " + inventaire.IDInventaire + " AND id_prod = @prod  AND det = '"+inventaire.IndexDetail +"'";
                    command.Transaction = _transaction;
                    ...
                    {
                        requete = "INSERT INTO det_invent (id, id_prod,qte, pc, pp,empla,ordre,qte_av ,det)" +
                            "VALUES(@idd, @id_prod,@qte,@pc,@pp, @empl,@ordre, " + inventaire.QuantiteAvant +",'" + inventaire.IndexDetail + "')";
                    }
                    else
                    {
                        requete = "UPDATE det_invent SET qte=@qte,pc=@pc, pp=@pp, empla=@empl,ordre=@ordre  WHERE  id=@idd AND id_prod=@id_prod AND det='" + inventaire.IndexDetail+"'";
                    }
        ...
        public static bool ValiderInventaire(DataGridView dgvInventaire)
        {
            try
            {
                ...
                foreach (DataGridViewRow dgv in dgvInventaire.Rows)
                {
                    ...
                    var indexDetail = Convert.ToInt32(dgv.Cells[9].Value.ToString());
                    ...
                    else
                    {
                         ...
                         _requete = string.Format("UPDATE medicament SET nbreDetail = {0}, prixDetail = {1} WHERE (num_medi ='{2}')",
                             produit.Quantite, produit.PrixPublic,produit.IDProduit);
        ...
        public static List<Inventaire> ListeDetailsInventaire()
        {
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    p.IndexDetail = !reader.IsDBNull(8) ? reader.GetString(8) : "0";
       ...
       //creer un produit
        ...
        public static DataTable JournalDesLivraisons(DateTime dateDebut, DateTime dateFin, string nomFournisseur)
        {
            try
            {
                string requete = "SELECT detail_bl.* from detail_bl INNER JOIN fournisseur ON detail_bl.num_fourn=fournisseur.num_fourn" +
                    " WHERE detail_bl.date_bl >=@date1 AND detail_bl.date_bl < @date2 AND fournisseur.nom_fourn =@nom ORDER BY detail_bl.date_bl, detail_bl.num_bl";
        ...
        public static DataTable RapportDeLivraison(string numLivraison)
        {
            ...
            try
            {
                string requete = "SELECT  medicament.nom_medi,bn_liv.quantite,bn_liv.prix_achat," +
                                 " bn_liv.prix_total FROM bn_liv INNER JOIN medicament ON bn_liv.num_medi" +
                                 "  = medicament.num_medi WHERE bn_liv.num_bl = @num ORDER BY  medicament.nom_medi";
