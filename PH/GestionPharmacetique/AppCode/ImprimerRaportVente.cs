...
namespace GestionPharmacetique.AppCode
{
     ...
     public class ImprimerRaportVente
     {
       static   System.Globalization.CultureInfo elGR = System.Globalization.CultureInfo.CreateSpecificCulture("fr-FR");
         ...
         public static Bitmap ImprimerRapportDesConsommations(string titre, System.Windows.Forms.DataGridView dgvRapport)
         {
             //les dimension de la facture
             #region
             int unite_hauteur = 20;
             ...
             graphic.DrawString("Rapport de consommations des services" + titre, fnt1, Brushes.Black, unite_largeur, 7 * unite_hauteur - 10);

             graphic.FillRectangle(Brushes.SteelBlue, unite_largeur, 8 * unite_hauteur - 2, 23 * unite_largeur, unite_hauteur);
             graphic.DrawString("SERVICE".ToUpper(), fnt1, Brushes.White, unite_largeur * 5, 8 * unite_hauteur);
             graphic.DrawString("MONTANT ".ToUpper(), fnt1, Brushes.White, 15 * unite_largeur + 5, 8 * unite_hauteur);

             var montantTotalVente = 0.0;
             var j = 0;
             for (var i = 0; i <= dgvRapport.Rows.Count - 1; i++)
             {
                 var Yloc = unite_hauteur * j + 9 * unite_hauteur;
             ...
             var LOC = unite_hauteur * j + 10 * unite_hauteur; 
             for (var l = 0; l < dgvRapport.Rows.Count; l++)
             {
                 montantTotalVente += Double.Parse(dgvRapport.Rows[l].Cells[1].Value.ToString());
             }
             //graphic.FillRectangle(Brushes.White, unite_largeur, LOC, 25 * unite_largeur + 15, 10 * unite_hauteur);
            graphic.FillRectangle(Brushes.SteelBlue, unite_largeur, LOC, 25 * unite_largeur + 15, unite_hauteur + 5);
            graphic.DrawString("Montant total  ", fnt11, Brushes.White, unite_largeur, LOC);
             graphic.DrawString(string.Format(elGR, "{0:0,0}", montantTotalVente) + " F.CFA", fnt11, Brushes.White, 18 * unite_largeur, LOC);
    ...
    dtGrid.Rows[i].Cells[2].Value.ToString().ToUpper() == "TOTAL GENERAL")
        ...
        public static Bitmap ImprimerJournalDesLivraisons(System.Windows.Forms.DataGridView dgvInventaire,string titre, string montantTotal, int index)
        {
            ...
            //definir les polices 
            Font fnt1 = new Font("Arial Unicode MS", 11, FontStyle.Regular);
            Font fnt11 = new Font("Arial Unicode MS", 10.5f, FontStyle.Bold);
            Font fnt3 = new Font("Arial Unicode MS", 16, FontStyle.Bold);
            Font fnt33 = new Font("Arial Unicode MS", 10, FontStyle.Regular);
            try
            {
                Image logo = global::GestionPharmacetique.Properties.Resources.logo;
                graphic.DrawImage(logo, unite_largeur, unite_hauteur + 5, 12 * unite_largeur, 4 * unite_hauteur);
            }
            catch { }
            // dessiner les ecritures 
            ...
            graphic.DrawString("N° facture".ToUpper(), fnt11, Brushes.White, 17 * unite_largeur-16, 8 * unite_hauteur);
            graphic.DrawString("Montant".ToUpper(), fnt11, Brushes.White, 20 * unite_largeur, 8 * unite_hauteur);
            ...
            for (int i = index * 45; i < dgvInventaire.Rows.Count; i++)
            {
                ...
                if (dgvInventaire.Rows[i].Cells[0].Value.ToString().ToUpper() == "SOUS TOTAL" ||
                    dgvInventaire.Rows[i].Cells[0].Value.ToString().ToUpper() == "TOTAL")
                {
                    ...
                    graphic.DrawString(dgvInventaire.Rows[i].Cells[3].Value.ToString() + ".F", fnt11, Brushes.Black, 20 * unite_largeur, Yloc);
                ...
                else
                {
                    ...
                    graphic.DrawString(dgvInventaire.Rows[i].Cells[2].Value.ToString(), fnt1, Brushes.Black, 17 * unite_largeur-16, Yloc);
                    graphic.DrawString(dgvInventaire.Rows[i].Cells[3].Value.ToString(), fnt1, Brushes.Black, 20 * unite_largeur, Yloc);
        ...
        //fonction pour dessiner le stock inventaire
        public static Bitmap ImprimerStatistique(System.Windows.Forms.ListView lstView, string titre, int index)
        {
            #region
            int unite_hauteur = 20;
            int unite_largeur = 32;
            ...
            {
                Image logo = global::GestionPharmacetique.Properties.Resources.logo;
                graphic.DrawImage(logo, unite_largeur, unite_hauteur + 5, 12 * unite_largeur, 3 * unite_hauteur);
            }
            catch { }
            ...
            graphic.DrawString("Stock".ToUpper(), fnt0, Brushes.Black, 13 * unite_largeur + 5, 8 * unite_hauteur-2);
            graphic.DrawString("Qté".ToUpper(), fnt0, Brushes.Black, 15 * unite_largeur+5 , 8 * unite_hauteur-2);
            graphic.DrawString("Prix".ToUpper(), fnt0, Brushes.Black, 16 * unite_largeur + 21, 8 * unite_hauteur-2);
            graphic.DrawString("Px total".ToUpper(), fnt0, Brushes.Black, 19 * unite_largeur +5, 8 * unite_hauteur-2);
            graphic.DrawString("% vente".ToUpper(), fnt0, Brushes.Black, 22 * unite_largeur - 11, 8 * unite_hauteur-2);

            var j = 0;
            var total = .0;
            for (int i = 45*index; i < lstView.Items.Count; i++)
            {
                int Yloc = unite_hauteur * j + 9 * unite_hauteur;
                graphic.DrawRectangle(Pens.Black, unite_largeur, Yloc, unite_largeur * 12 - 3, unite_hauteur);
                graphic.DrawRectangle(Pens.Black, unite_largeur * 13, Yloc, unite_largeur * 2 - 3, unite_hauteur);
                graphic.DrawRectangle(Pens.Black, unite_largeur * 15, Yloc, unite_largeur * 2 - 18, unite_hauteur);
                ...
                graphic.DrawString(lstView.Items[i].SubItems[0].Text, fnt1, Brushes.Black, unite_largeur + 10, Yloc);
                graphic.DrawString(lstView.Items[i].SubItems[1].Text, fnt1, Brushes.Black, 13 * unite_largeur + 5, Yloc);
                graphic.DrawString(lstView.Items[i].SubItems[2].Text, fnt1, Brushes.Black, 15 * unite_largeur + 5, Yloc);
                graphic.DrawString(lstView.Items[i].SubItems[3].Text, fnt1, Brushes.Black, 16 * unite_largeur + 28, Yloc);
                graphic.DrawString(lstView.Items[i].SubItems[4].Text, fnt1, Brushes.Black, 19 * unite_largeur + 5, Yloc);
                graphic.DrawString(lstView.Items[i].SubItems[5].Text, fnt1, Brushes.Black, 22 * unite_largeur , Yloc);
                total += Convert.ToDouble(lstView.Items[i].SubItems[4].Text);
                j++;
            }
            if(index ==0)
                graphic.DrawString(total.ToString(), fnt0, Brushes.Black, 22*unite_largeur -2, 7 * unite_hauteur - 5);
            graphic.FillRectangle(Brushes.White, unite_largeur  - 16, 54*unite_hauteur+2, unite_largeur * 24 + 17, unite_hauteur*3);
