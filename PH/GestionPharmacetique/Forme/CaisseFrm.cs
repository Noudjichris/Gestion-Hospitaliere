...
namespace GestionPharmacetique.Forme
{
    public partial class CaisseFrm : Form
    {
        ...
        private void RapportCaisseFrm_Load(object sender, EventArgs e)
        {
            try
            {
                //DataTable dt = AppCode.ConnectionClass.ListeDistinctVenteDesVentes(dateDebut, dateDebut);
                var listeEMploye = from f in AppCode.ConnectionClass.ListeDesEmployees()
                                   //where f.Titre.ToLower().Contains("caiss")
                ...
                comboBox2.Text= "<Tous les caissiers>";
                //foreach (DataRow dtRow in dt.Rows)
                //{                
                //    comboBox2.Items.Add(dtRow.ItemArray[0].ToString());
                //}
                if (siVenteTotal)
                {
