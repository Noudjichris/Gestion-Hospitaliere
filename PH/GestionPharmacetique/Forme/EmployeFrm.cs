...
namespace GestionPharmacetique.Forme
{
    public partial class EmployeFrm : Form
    {
                   ...
                   ! string.IsNullOrEmpty(txtVille.Text))
                {
                    ...
                    if (txtNomEmploye.Text.Contains(" "))
                    {
                                                                    ...
                                                                    txtEmail.Text.Contains(".")))
                        {
                            ...
                            {
                                typeUtilisateur = "intermediaire";
                            }
                            else if (radioButton4.Checked)
                            {
                                typeUtilisateur = "moyen";
                            }
                            else
                            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if(txtUtilisateur.Text!=null && lblMotDePasse.Text !=null)
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    if (MonMessageBox.ShowBox("Voulez vous reinitialiser le mot de passe de " + txtNomEmploye.Text + "?", "Confirmation", "confirmation.png") == "1")
                    {
                        ConnectionClass.ModifierMotDePasse(txtUtilisateur.Text,
                             lblMotDePasse.Text.ToString(), txtNumeroEmploe.Text.Trim().GetHashCode().ToString());
                    }
                }
            }
            catch { }
