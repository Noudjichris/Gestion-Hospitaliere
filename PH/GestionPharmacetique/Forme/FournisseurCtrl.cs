...
namespace SGDP.Formes
{
    public partial class FournisseurCtrl : UserControl
    {
        ...
        {
            btnExit.Location = new Point(Width - 45, btnExit.Location.Y);
        ...
        void ListeFournisseur()
        {
            try
            {
                ...
                var liste = from l in ConnectionClass.ListeFournisseur()
                            where l.ID>0
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (dgvFournisseur.SelectedRows.Count > 0)
                {
                    if (GestionPharmacetique. MonMessageBox.ShowBox("Voulez vous supprimer les données de ce fournisseur?", "Confirmation", "confirmation.png") == "1")
                    {
                        ConnectionClass.InsererDansLog(GestionPharmacetique.Form1.nomEmploye, "Suppression des données du fournisseur", this.Name);
