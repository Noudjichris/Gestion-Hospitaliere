        ...
        private void viderLaListeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (MonMessageBox.ShowBox("Voulez vous commencer une nouvelle inventaire?", "Confirmation", "confirmation.png") == "1")
                {
                    ...
                    txtNoInv.Text = ConnectionClass.EnregistrerInventaire(listeProduit).ToString();
                    foreach (var  produit in listeProduit)
                    {
                        dgvProduit.Rows.Add(
                            ...
                            if(produit.NombreDetail>0 && produit.PrixVenteDetail>0)
                            { 
                                dgvProduit.Rows.Add(
                            produit.NumeroMedicament,
                            produit.NomMedicament.ToUpper() + " DETAIL",
                            (int)produit.PrixAchat / produit.NombreBoite,
                            produit.PrixVenteDetail, "",
                            produit.NombreDetail,"",
                            txtEmplacement.Text, "",
                            produit.DateExpiration.ToShortDateString(),"",""
                            );
                            }
                    ...
                    if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        for (var i = 0; i <= index; i++)
                        {
                            ...
                            {
                                var _inventaireStock = AppCode.Imprimer.ImprimerInventaireStockPage(dgvProduit, i, DateTime.Now,0,false);
                    ...
                    if (txtNoInv.Text != "")
                    {
                        var listeProduit = from i in ConnectionClass.ListeDetailsInventaire()
                                           ...
                                           select new
                                           {
                                               ...
                                               i.QuantiteAvant,
                                               i.Emplacement,
                                               p.DateExpiration,i.IndexDetail,i.Ordre
                                           };
        ...
        private void validerLinventaireToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvProduit.Rows.Count > 0)
                    ...
                    {
                        if (ConnectionClass.ValiderInventaire(dgvProduit))
                        {
            ...
            try
            {
                ...
                if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (var i = 0; i <= index; i++)
                    {
                        if (i * 45 < dgvProduit.Rows.Count)
                        {
                            var _inventaireStock = AppCode.Imprimer.ImprimerInventaireStockPage(dgvProduit, i, dateInv,total, true);
                ...
                if (txtNoInv.Text != "")
                {
                    ...
                    foreach (var p in listeProduit)
                    {
                        ...
                        dgvProduit.Rows.Add(
                                            ...
                                            ecart,
                                            p.DateExpiration.ToShortDateString(),
                                             p.Emplacement, p.IndexDetail, p.ID
