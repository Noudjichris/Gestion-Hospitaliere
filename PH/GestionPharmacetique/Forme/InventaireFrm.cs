    ...
    public partial class InventaireFrm : Form
    {
        ...
        private void InventaireFrm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                listView1.Columns.Add("GRD STOCK", listView1.Width / 8);
                ...
                label3.Location = new Point(4*listView1.Width/8-20, this.label3.Location.Y);
                label4.Location = new Point(6 * listView1.Width / 8-20, this.label4.Location.Y);
                lblResultatNet.Location = new Point(7 * listView1.Width / 8-20, this.label4.Location.Y);
                decimal prixCT = 0;
                decimal prixPT = 0;
                var resultatNet = 0m;
                ...
                {   int qte;
                    ...
                    else if (comboBox2.Text == "Grand Stock")
                    {
                ...
                label2.Text = "Nombre de materiels inventorié : " + listView1.Items.Count;
                label3.Text =  prixCT + " F.CFA";
                label4.Text =  prixPT + " F.CFA";
                lblResultatNet.Text = resultatNet.ToString() + " F.CFA";
            }
...
frm.dataGridView1.Rows.Add(
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try {
                ...
                decimal prixPT = 0;
                var resultatNet = 0m;
                var listeArrayList = ConnectionClass.ListeDesMedicamentsRechercherParFamille(comboBox1.Text);
                if (comboBox1.Text == "<TOUTES>")
                {
                    listeArrayList = ConnectionClass.ListeDesMedicamentsRechercherParFamille("");
                }
                foreach (Medicament medicament in listeArrayList)
                {
                    int qte;
                    ...
                    ppt = (medicament.GrandStock+medicament.Quantite) * medicament.PrixVente;
                    pct = (medicament.GrandStock + medicament.Quantite) * medicament.PrixAchat;
                    var items = new string[]
                    {
                     medicament.NomMedicament, 
                     medicament.NumeroMedicament ,                    
            ...
            if (checkBox1.Checked)
            {
                try
                {
                    ...
                    foreach (Medicament medicament in listeArrayList)
                    {
                        ...
                        ppt = (medicament.GrandStock + medicament.Quantite) * medicament.PrixVente;
                        pct = (medicament.GrandStock + medicament.Quantite) * medicament.PrixAchat;
                        if (medicament.Quantite > 0 || medicament.GrandStock > 0)
                        {
                            var items = new string[]
                    {
                     medicament.NomMedicament, 
