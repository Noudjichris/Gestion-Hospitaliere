    ...
    public partial class LivraisonFournisseurFrm : Form
    {
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblEtat.Text = ConnectionClass.VerifierEtatLivraison(Convert.ToInt32(txtNoLivraisom.Text)).ToString();
                ...
                if (sfd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    for (var i = 0; i <= index; i++)
                    {
                        if (i * 43 < count)
                        {
                            ...
                            {
                                if (dgvVente.Rows[dgvVente.Rows.Count - 1].Cells[2].Value.ToString() != "TOTAL")
                                {
                                    dgvVente.Rows.Add(
                                    "0", "0",
                           "TOTAL", "", "", "", txtMontantTTC.Text, "", "");
                                }
                                var flag = false;
                                if (MonMessageBox.ShowBox("Voulez vous imprimer le bon de commande avec le stock actuel", "Confirmation", "confirmation.png") == "1")
                                {
                                    flag = true;
                                }
                               _listeImpression= Impression.ImprimerBonDeCommande
                                    (dgvVente, txtNoLivraisom.Text, cmbFournisseur.Text, dateTimePicker1.Value.ToShortDateString(),flag, i);
                            }
        ...
        private void cmbAnnee_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (Livraison livraison in livraisonArrayList)
                {
                    if(livraison.DateCommande.Year==int.Parse(cmbAnnee.Text))
                    comboBox1.Items.Add(livraison.NumeroCommande + " & " + livraison.DateCommande.ToShortDateString());
                }
            }
            catch (Exception ex) { MonMessageBox.ShowBox("", ex); }
        ...
        private void txtFacture_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                try
