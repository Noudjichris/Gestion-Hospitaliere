       ...
       private void groupBox4_Paint(object sender, PaintEventArgs e)
        ...
        private void MedicamentFrm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Form1.typeUtilisateur == "admin" && Form1.typeUtilisateur == "intermediaire")
                {
           ...
           if (!string.IsNullOrEmpty(txtNomMedicament.Text))
            {
                ...
                {
                    if ( decimal.TryParse(txtPrVente.Text, out _prixVente))
                    {
                            ...
                            if (!string.IsNullOrEmpty(comboBox1.Text))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (Int32.TryParse(txtSeuil.Text, out _qteAlerte))
                                    {
            ...
            try
            {
                ...
                //if (cmbTrie.Text == "Nom" || string.IsNullOrEmpty(cmbTrie.Text))
                {
                    ...
                    foreach (Medicament produit in listeMedicament)
                    {
                        ...
                        var stock = produit.GrandStock + produit.Quantite;
                        var stockMinimal = produit.QuantiteAlerte;
                        
                        if (stock <= 0)
                        {
                           dgvProduit.Rows[dgvProduit.Rows.Count - 1].DefaultCellStyle.ForeColor = Color.Red;
                        }
                        else if (stock > 0 && stock < stockMinimal)
                        {
                            dgvProduit.Rows[dgvProduit.Rows.Count - 1].DefaultCellStyle.ForeColor = Color.Orange;
                        }
                       ConnectionClass. MettreAjourLotDeProduit(produit.NumeroMedicament);
                    }
                }  
