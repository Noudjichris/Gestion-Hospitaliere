        ...
        private void RechercheMedicamentIndiFrm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                int qteTotaleVendue = 0, qteTotaleRetournee = 0, qteTotaleLivree = 0,
                    quantiteTotaleEntree=0,quantiteTotaleSortie=0;
                foreach (var date in listeJour)
                {
                    ...
                    var listeSortie = from m in AppCode.ConnectionClass.ListeMouvementStock(1)
                                      join f in AppCode.ConnectionClass.LogStock()
                                on m.ID equals f.GroupeID
                                      join p in AppCode.ConnectionClass.ListeDesMedicamentsRechercherParNom("")
                              on f.NumeroProduit equals p.NumeroMedicament
                                where m.Date >= date
                                where m.Date < date.AddHours(24)
                                where m.Etat==2
                                where p.NomMedicament.StartsWith(designation, StringComparison.CurrentCultureIgnoreCase)
                                orderby p.NomMedicament
                                select new
                                {
                                    f.DifferenceStock
                                };
                    var listeEntree = from  m in AppCode.ConnectionClass.ListeMouvementStock(1)
                                      join f in AppCode.ConnectionClass.LogStock()
                                on m.ID equals f.GroupeID
                                      join p in AppCode.ConnectionClass.ListeDesMedicamentsRechercherParNom("")
                              on f.NumeroProduit equals p.NumeroMedicament
                    ...
                    var count = dtVente.Rows.Count + dtRetour.Rows.Count + dtLivraison.Rows.Count+listeEntree.Count()+listeSortie.Count();
                    ...
                    var qteEntree = 0;
                    var detaille = 0;
                  foreach(var i in listeSortie)
                  {
                      qteSortie +=i.DifferenceStock;
                      quantiteTotaleSortie += i.DifferenceStock;
                  }
                       foreach(var i in listeEntree)
                  {
                      qteEntree +=i.DifferenceStock;
                      quantiteTotaleEntree += i.DifferenceStock;
                  }
                    ...
                    if (qteSortie > 0)
                    {
                        quantiteSortie = qteSortie.ToString();
                    }
                    var quantiteEntree = "";
                    if (qteEntree > 0)
                    {
                        quantiteEntree = qteEntree.ToString();
                    }
                    if (count > 0)
                    {
                        if (dtVente.Rows.Count == 0)
                        {
                            dgvVente.Rows.Add( date.ToShortDateString(), qteLivree , qteRetourne ,quantiteEntree,quantiteSortie, "","");
                        }
                ...
                dgvVente.Rows.Add("Nbres totaux", qteTotaleLivree,qteTotaleRetournee, quantiteTotaleEntree, quantiteTotaleSortie, "", qteTotaleVendue);
