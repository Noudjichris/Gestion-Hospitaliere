...
namespace GestionPharmacetique.Forme
{
    public partial class RetourLivrFrm : Form
    {
                 ...
                 int qte;double prix;
                    if (Int32.TryParse(txtQte.Text, out qte) && double.TryParse(txtPrixCession.Text , out prix))
                    {
                        ...
                        if (btnClick == "1")
                        {
                            ...
                            AppCode.ConnectionClass.InsererLeRetour(numeroProduit, txtDesignation.Text, prix, qte, txtMotif.Text, idLiv,noLot);
                            txtQte.Text = "";
                            txtMotif.Text = "";
                            txtDesignation.Text = "";
                            txtPrixCession.Text = "";
       ...
       public  string numeroProduit;
        private void txtDesignation_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    ListeProduitAvecLot.indexRecherche = txtDesignation.Text;
                    if(txtDesignation.Text.Length>2)
                    if (ListeProduitAvecLot.ShowBox() == "1")
                    {
                        noLot = ListeProduitAvecLot.NoLot;
                        var flag = false;
                        var livraisonArrayList = AppCode.ConnectionClass.ListeDesDetailCommandes(lblBonLivraison.Text);
                        foreach (AppCode.Livraison livraison in livraisonArrayList)
                        {
                            if (livraison.Designation.ToUpper() == ListeProduitAvecLot.designation.ToUpper())
                            {
                                flag = true;
                            }
                        }
                        ...
                        {
                            var montant = 0m;
                            txtDesignation.Text = ListeProduitAvecLot.designation;
                            txtPrixCession.Text = String.Format(elGR, "{0:0,0}", ListeProduitAvecLot.prixCession);
                            txtPrixTotal.Text = String.Format(elGR, "{0:0,0}", montant);
                            numeroProduit = ListeProduitAvecLot.numeroProduit;
                            txtQte.Focus();
                        }
                        else
                        {
        ...
        {
            var listeLivraison = AppCode.ConnectionClass.ListeDesLivraisons(Convert.ToInt32(lblBonLivraison.Text));
        ...
        private void txtPrixCession_TextChanged(object sender, EventArgs e)
        {
            txtQte_TextChanged(null, null);
        }
