    ...
    public partial class VenteFrm : Form
    {
      ...
      private void groupBox10_Paint(object sender, PaintEventArgs e)
        ...
        private void VenteFrm_Load(object sender, EventArgs e)
        {
            try
            {
                // TODO: This line of code loads data into the 'pHDS.detail_bl' table. You can move, or remove it, as needed.
                //this.detail_blTableAdapter.Fill(this.pHDS.detail_bl);
                var widthControle = (groupBox3.Width - 180) / 4 - 3;
                groupBox4.Location = new Point(groupBox3.Width + 30, groupBox4.Location.Y);
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (etat == "4")
                {
                    _listeImpression = Impression.ImprimerFacture
                                 (numVente, dgvVente, client, dateTimePicker1.Value , 
                                 "CREDIT N° ", "Credit pris le : ", Double.Parse(txtTotalRemise.Text));
                    if (printDialog1.ShowDialog() == DialogResult.OK)
                    {
                        printDocument1.PrinterSettings = printDialog1.PrinterSettings;
                        printPreviewDialog1.ShowDialog();
        ...
        {
            e.Graphics.DrawImage(_listeImpression, -15, -10, _listeImpression.Width, _listeImpression.Height);
        ...
        private void btnCrediter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvVente.Rows.Count > 0)
                {
                    ...
                    {
                        MonMessageBox.ShowBox("Veuillez entrer le nom du client puis réessayez.", "Erreur ", "erreur.png");
                        return;
                    }
                    ...
                    List<Employe> listeEmploye = ConnectionClass.ListeDesEmployees("nom_empl", "'" + Form1.nomEmploye + "'");
                    string numEmploye =  listeEmploye[0].NumMatricule;
                    var dateVente = dateTimePicker1.Value;
                    var heure = DateTime.Now;
                    var vente = new Vente();
                    vente.SiVente = false;
                    vente.DateVente = dateVente;
                    ...
                    if (MonMessageBox.ShowBox("Voulez vous enregistrer ce credit ?", "Demande confirmation", "confirmation.png") == "1")
                    {
                        ...
                        if (ConnectionClass.EnregistrerVente(vente, dgvVente, client))
                        {
                            ...
                            _listeImpression = Impression.ImprimerFacture
                                   ...
                                   "CREDIT NO ", "Credit pris le : ", Double.Parse(txtTotalRemise.Text));
                            if (printDialog1.ShowDialog() == DialogResult.OK)
                            {
                                printDocument1.PrinterSettings = printDialog1.PrinterSettings;
                                printDocument1.Print();
        ...
        private void dgvVente_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                txtRechercherProduit_KeyDown(null, null);
            }
        }
            ...
            try
            {
                ...
                if (listeDistributionFrm.ShowBox() == "1")
                {
                    ...
                    foreach (DataRow dtRow in dt.Rows)
                    {
                        ...
                        if (decimal.Parse(dtRow.ItemArray[3].ToString()) * 2 < liste[0].PrixVente)
                        {
                            designation = designation + " DETAIL";
                        }
                        var tauxRemise = 100 - 100 * double.Parse(dtRow.ItemArray[5].ToString()) / double.Parse(dtRow.ItemArray[3].ToString()) * double.Parse(dtRow.ItemArray[4].ToString());
                        dgvVente.Rows.Add(
                            dtRow.ItemArray[0].ToString(),
                            designation ,
