...
namespace GestionPharmacetique.AppCode
{
    class ConnectionClassClinique
    {
        ...
        public static DataTable ListeDesEntreprises(string nomEntreprise)
        {
            ...
            try
            {
                requete = "SELECT * FROM entre_tbl ORDER BY entreprise WHERE entrep = '"+nomEntreprise+"'";
                command.CommandText = requete;
                connection.Open();
                var reader = command.ExecuteReader();
                dt.Load(reader);
        ...
        public static DataTable ListeDesAnalyses(String analyse)
        {try
            {
                requete = "SELECT * FROM liste_analyse_tbl WHERE type_anal LIKE '%" +analyse +"%' ORDER BY type_anal";
                command.CommandText = requete;
                connection.Open();
                var reader = command.ExecuteReader();
