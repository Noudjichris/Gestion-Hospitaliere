   ...
   public  class Imprimer
    {
       ...
       public static Bitmap ImprimerRapportDepenses(ListView lstDepenses, string titre, int start)
            ...
            int index, DateTime dt, double total, bool flag)
        {
            ...
            graphic.FillRectangle(Brushes.Lavender, unite_largeur * 20-10, 7 * unite_hauteur - 5, unite_largeur * 2 -5, unite_hauteur + 8);
            graphic.FillRectangle(Brushes.Lavender, unite_largeur * 22 - 12, 7 * unite_hauteur - 5, unite_largeur * 2-5, unite_hauteur + 8);

            graphic.DrawString("N°".ToUpper(), fnt11, Brushes.Black, 20, 7 * unite_hauteur - 3);
            graphic.DrawString("designation".ToUpper(), fnt11, Brushes.Black, unite_largeur * 2, 7 * unite_hauteur - 3);
            graphic.DrawString("Expire le".ToUpper(), fnt11, Brushes.Black, 11 * unite_largeur + 15, 7 * unite_hauteur - 3);
            graphic.DrawString("STOCK \n PREC".ToUpper(), fnt11, Brushes.Black, 14 * unite_largeur + 10, 7 * unite_hauteur - 3);
            graphic.DrawString("STOCK \n FINAL".ToUpper(), fnt11, Brushes.Black, 16 * unite_largeur + 10, 7 * unite_hauteur - 3);
            graphic.DrawString("Ecart".ToUpper(), fnt11, Brushes.Black, 18 * unite_largeur + 10, 7 * unite_hauteur - 1);
            graphic.DrawString("PRIX\nACHAT".ToUpper(), fnt11, Brushes.Black, 20 * unite_largeur + 10, 7 * unite_hauteur - 3);
            graphic.DrawString("PRIX\nTOTAL".ToUpper(), fnt11, Brushes.Black, 22 * unite_largeur -10, 7 * unite_hauteur - 3);
            ...
            for (int i = index * 45; i < dgvInventaire.Rows.Count; i++)
            {
                ...
                {
                    if (dgvInventaire.Rows[i].DefaultCellStyle.BackColor == Color.AliceBlue)
                    {
                        graphic.FillRectangle(Brushes.Lavender, 15, Yloc + 1, unite_largeur * 23 + 2, unite_hauteur - 2);
                        graphic.DrawString(dgvInventaire.Rows[i].Cells[1].Value.ToString(), fnt3, Brushes.Black, unite_largeur * 9 + 10, Yloc);

                    }
                    else
                    {
                        graphic.FillRectangle(Brushes.WhiteSmoke, 15, Yloc + 1, unite_largeur * 23 + 2, unite_hauteur - 2);
                ...
                else
                {
                    ...
                    graphic.DrawRectangle(Pens.Black, unite_largeur * 20 - 10, Yloc, unite_largeur * 2 - 5, unite_hauteur + 0);
                    graphic.DrawRectangle(Pens.Black, unite_largeur * 22 - 12, Yloc, unite_largeur * 2 - 5, unite_hauteur + 0);
                    ...
                    if (flag)
                    {
                        ...
                        graphic.DrawString(dgvInventaire.Rows[i].Cells[2].Value.ToString(), fnt1, Brushes.Black, 20 * unite_largeur + 0, Yloc);
                        graphic.DrawString((double.Parse(dgvInventaire.Rows[i].Cells[2].Value.ToString()) * 
                            double.Parse(dgvInventaire.Rows[i].Cells[4].Value.ToString())).ToString(), fnt1, Brushes.Black, 22 * unite_largeur + 0, Yloc);
                    }
                    numero++;
                }
        ...
        public static Bitmap ImprimerInventaireStockAnnuel(string titre, DataGridView dgvView , int start, string depot)
        {
            ...
            {
                Image logo = global::GestionPharmacetique.Properties.Resources.logo;
                graphic.DrawImage(logo, unite_largeur, unite_hauteur + 5, 24 * unite_largeur, 3 * unite_hauteur + 15);
            }
            catch { }
             ...
             int index, DateTime dt, double total, bool flag)
       {
           ...
           try
           {
               ...
               if (flag)
               {
                   ...
                   { 
                       var totalVente = 0m;
                       var totalAchat = 0m;
                       for (int i = 0; i < dgvInventaire.Rows.Count; i++)
                       {
                           decimal prixAchat, prixVente, quantite;
                           if (decimal.TryParse(dgvInventaire.Rows[i].Cells[2].Value.ToString(), out prixAchat) &&
                               decimal.TryParse(dgvInventaire.Rows[i].Cells[3].Value.ToString(), out prixVente) &&
                               decimal.TryParse(dgvInventaire.Rows[i].Cells[6].Value.ToString(), out quantite))
                           {
                               totalAchat += prixAchat * quantite;
                               totalVente += prixVente*quantite;
                           }
                       }
                       graphic.FillRectangle(Brushes.Lavender, unite_largeur, 54 * unite_hauteur + 1, unite_largeur * 26, unite_hauteur);
                       graphic.DrawString("Stock total", fnt1, Brushes.Black, 2 * unite_largeur, 54 * unite_hauteur + 2);
           ...
           {
               MonMessageBox.ShowBox("evaluer", ex);
               return null;
           }
       } 
   }
