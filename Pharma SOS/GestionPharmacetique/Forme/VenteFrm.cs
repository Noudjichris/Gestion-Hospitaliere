      ...
      private void groupBox10_Paint(object sender, PaintEventArgs e)
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal paieTotal = 0;
                if (txtPaye.Text.Contains(","))
                {
                    paieTotal = decimal.Parse(txtPaye.Text.Substring(0, txtPaye.Text.LastIndexOf(",")));
                }
                else
                {
                    paieTotal += decimal.Parse(txtPaye.Text);
                }
                if (etat == "2")
                {
                        _listeImpression = Impression.ImprimerFacture
                                    (numVente, dgvVente, client, dateTimePicker1.Value,
                                    "FACTURE N° ", "Règlement en èspeces le : ", Double.Parse(txtTotalRemise.Text));
                ...
                else
                {
                    ...
                    var dateVente = DateTime.Now.Date ;
                        ...
                        {
                            double re;
                            if(double.TryParse(txtRemise.Text, out re))
                            {
                            }
                            else
                            {
                                MonMessageBox.ShowBox("Veuillez entrer la part de l'assureur et taper sur entrer", "Erreur", "erreur.png");
                                return;
                            }
                            ...
                            if (!string.IsNullOrEmpty(dataGridView2.SelectedRows[0].Cells[7].Value.ToString()))
                            {
                                ...
                                {
                                    MonMessageBox.ShowBox("Ce client n'est pas couvert", "Erreur", "erreur.png");
                                }
                            }
                            else
                            {
                                MonMessageBox.ShowBox("Veuillez entrer selectionner le nom du client sur la liste", "Erreur", "erreur.png");
                            }
                        }
                    ...
                    vente.TotalPaye = paieTotal;
                    var part = .0;
                    if (double.TryParse(txtRemise.Text, out part))
                    { }
                    vente.PartAssure = part;
                    if (ConnectionClass.EnregistrerVente(vente, dgvVente, client, chkRemise.Checked, checkBox4.Checked))                   
                    {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawImage(_listeImpression, -0, -10, _listeImpression.Width, _listeImpression.Height);
            ...
            try
            {
                if (dgvVente.Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(txtPrixTotal.Text))
                    {
                        ...
                        txtPaye.Text = montantTh.ToString();
                    }
                }
            }
            catch (Exception )
                            ...
                            {  
                                  dataGridView2.Rows.Clear();
                                    ...
                                    _listeImpression = Impression.ImprimerFacture
                                           ...
                                           "FACTURE NO ", "Date : ", Double.Parse(txtTotalRemise.Text));
        ...
        private void txtRechercherProduit_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if ( e.KeyCode == Keys.End)
                {
                    ...
                    txtPaye.Text = (montantTh - totalRemise).ToString();
                    //btnEnregistrer.Enabled = true;
        ...
        private void dgvVente_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                var tauxRemise = Double.Parse(dgvVente.Rows[rowIndex].Cells[4].Value.ToString());
                    ...
                    if (!ConnectionClass.ValiderQuantitePharmacie(ListeProduitFrm.numeroProduit, qte))
                    {
                    ...
                    if (!string.IsNullOrEmpty(txtPrixTotal.Text))
                    {
                                ...
                                txtTotalRemise.Text = ((int)totalRemise).ToString();
                                txtPaye.Text = (((int)montantTh) - ((int)totalRemise)).ToString();
                    }
                txtRechercherProduit.Focus();
            }
            catch (Exception ex)
                ...
                try
                {
                    ...
                    if (Double.TryParse(txtRemise.Text, out tauxRemise))
                    {
                        ...
                        foreach (DataGridViewRow dgvRow in dgvVente.Rows)
                        {
                            ...
                            var prixTotal = Int32.Parse(dgvRow.Cells[6].Value.ToString());
                        ...
                        txtPaye.Text = (totalPrix - totalRemise).ToString();
                    }
                }
                catch (Exception ex)
                {
        ...
        private void chkRemise_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (dataGridView2.Rows.Count > 0)
            {
                if (string.IsNullOrEmpty(txtRecherche.Text))
                {
                    MonMessageBox.ShowBox("Veuillez d'abord selectionner le nom du client", "Erreur", "erreur.png");
                    chkRemise.Checked = false;
                    return;
                }
                else if (!string.IsNullOrEmpty(txtRecherche.Text)  && dataGridView2.SelectedRows[0].Cells[7].Value.ToString() != "1")
                {
                    MonMessageBox.ShowBox("Ce patient n'est pas sous couvert", "Erreur", "erreur.png");
                    chkRemise.Checked = false;
                    return;
                }
            }
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (listeDistributionFrm.ShowBox() == "1")
                {
                    ...
                    var partRemise = .0;
                    if (Double.TryParse(dt1.Rows[0].ItemArray[9].ToString(), out partRemise))
                    {
                    }
                    ...
                    txtTotalRemise.Text = ((int)total * partRemise / 100).ToString();
                    txtPaye.Text = (total - (double)total * partRemise / 100).ToString();
        ...
        {
            cmbConventione.Items.Clear();
            var dtEntrep = ConnectionClassClinique.ListeDesEntreprises(textBox1.Text);
            cmbConventione.Items.Add("");
            foreach (DataRow entrep in dtEntrep.Rows)
            {
                cmbConventione.Items.Add(entrep.ItemArray[1].ToString().ToUpper());
        ...
        private void textBox3_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgvVente.Rows.Count > 0)
                {
                    ...
                    var montantPaye = Decimal.Parse(textBox3.Text);
                    var ttc = Decimal.Parse(txtPaye.Text);
                    var reste = montantPaye - ttc;
                    if (reste >= 0)
                        lblResteCredit.Text = reste.ToString();
                    else
